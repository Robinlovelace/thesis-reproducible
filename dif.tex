...
%DIF DEL flat-old.tex   Sun Dec 22 19:46:11 2013
%DIF ADD flat-new.tex   Mon Dec 23 00:10:06 2013
%% Robin Lovelace's thesis (based on Sunil Patel's design from http://uk.tug.org/)
%% ----------------------------------------------------------------

% Improvements:
% Remove subsubsections, and maybe subsections also
% Add system-level energy costs!!! (Ch. 6)
% Add kWh/yr costs (Ch. 6) !!!

% NBs:
% Remember Ballas 2005b is Geo matters - use this, not 2005c
% Plan: update Chapter 3 when ints is up and published... expand on much of
% that stuff.
% Social trends statistical release: people travelling less far; 9.1%
% Try to make graphs replicable at the ended
% Add Lister's kW rating image as a graph comparing cars with other appliances
% Check the document for consistency - Chapter vs chapter, bullet points are
% full sentences? Do figures have lines? etc.
% classify areas: commuter belts account for x% of energy used
% Realistic savings if cycling carrying capacity were reached
% Realistic savings if all long-distance commutes were made less common
% Add to thesis-reproducible README file

% proportion of energy use consumed by cars - map - check!
% same thing for commuters: x% -> x% energy use

% What have I found out? answer this by Lotis.
% Calculate energy costs at system level!!!
...
%DIFDELCMD < Commuting is a daily 
%DIFDELCMD < ritual for a large proportion of the world's population.
%DIFDELCMD < It is important materially,
%DIFDELCMD < consuming large amounts of time, money and natural resources.
%DIFDELCMD < % Yet, as with many routine activities, travel to work is often
%DIFDELCMD < % taken for granted. In terms of inflexible energy use ---
%DIFDELCMD < % consumption that cannot easily be cut through reduced demand --- 
%DIFDELCMD < % commuting is of great interest.
%DIFDELCMD < As with many routine activities travel to work is often taken for
%DIFDELCMD < granted but its energy consumption is of particular interest due to
%DIFDELCMD < its heavy reliance on fossil fuels and
%DIFDELCMD < the inflexibility of the demand for commuting.
%DIFDELCMD < This understudied area of knowledge, the
%DIFDELCMD < energy costs of travel to work, forms the basis of the thesis.
%DIFDELCMD < 

%DIFDELCMD < % The variability of energy use for commuting over time has been
%DIFDELCMD < % indicative of wider shifts in personal travel. During the 20$^{th}$
%DIFDELCMD < % century the trend has been for trips made by foot, rail % too woolly! rm
%DIFDELCMD < % or bus to be replaced by the relatively energy intensive car.
%DIFDELCMD < % % This represents an `energy transition',
%DIFDELCMD < % % a long-term shift in the way that energy is used in the human economy.
%DIFDELCMD < % % Since the oil price shock of the summer 2008 and subsequent global economic
%DIFDELCMD < % % crisis, certain statistics suggest that this particular transition may have
%DIFDELCMD < % % gone into reverse. Car kilometres peaked in 2008, and show no signs of
%DIFDELCMD < % % returning to their former levels. What this means overall remains an open question:
%DIFDELCMD < % % more evidence on energy supplies, patterns of behaviour and the
%DIFDELCMD < % % ultimate unknown --- policy responses --- will be needed before any firm
%DIFDELCMD < % % conclusions can be drawn. Making judgements in the relatively narrow area
%DIFDELCMD < % % of travel to work is a far less ambitious
%DIFDELCMD < % The direction of change into the 21$^{st}$ century is uncertain, however,
%DIFDELCMD < % especially since car use has declined since 2008. Improved understanding of commuting
%DIFDELCMD < % patterns could therefore benefit efforts to reduce greenhouse gas emissions,
%DIFDELCMD < % and hence energy use, in the transport sector overall.
%DIFDELCMD < 

%DIFDELCMD < There is much research into commuting and transport energy use as separate
%DIFDELCMD < fields, but they have rarely been combined in the same analysis, let alone
%DIFDELCMD < at high levels of geographical resolution.
%DIFDELCMD < The well-established field of spatial microsimulation
%DIFDELCMD < offers tools for investigating commuting patterns in detail at local and
%DIFDELCMD < individual
%DIFDELCMD < levels, with major potential benefits for transport planning.
%DIFDELCMD < % In this project
%DIFDELCMD < % the method is applied, for the first time, to investigate variability in energy
%DIFDELCMD < % use both between and within small administrative zones.
%DIFDELCMD < For the first time this method is deployed to investigate variability in commuter energy
%DIFDELCMD < use both between and within small administrative zones.
%DIFDELCMD < % and
%DIFDELCMD < % the scenarios investigated in this PhD suggest that a wide range of phenomena,
%DIFDELCMD < % from future oil shocks to the localisation of economic activity could lead
%DIFDELCMD < % the energy use of commuting to fall. C
%DIFDELCMD < % ommuting is also
%DIFDELCMD < 

%DIFDELCMD < % The study most closely related to this thesis published to date, \citep{Boussauw2009},
%DIFDELCMD < % found that the energy costs of commuting are highly variable over space and to
%DIFDELCMD < % some extent predictable.
%DIFDELCMD < The maps of commuter energy use presented in this thesis
%DIFDELCMD < illustrate this variability at national, regional and local levels.
%DIFDELCMD < Supporting previous research, the results suggest that a
%DIFDELCMD < range of geographical factors influence energy use for travel.
%DIFDELCMD < This
%DIFDELCMD < has important policy implications: when high transport energy use in commuting
%DIFDELCMD < is due lack of jobs in
%DIFDELCMD < the vicinity, for example, modal shift (e.g.~from cars to bicycles)
%DIFDELCMD < on its own has a limited potential to reduce
%DIFDELCMD < energy costs. Such insights are quantified using existing aggregate data.
%DIFDELCMD < The main methodological contribution of this work, however, is to add
%DIFDELCMD < individual-level factors to
%DIFDELCMD < the analysis --- creating the potential for policy makers to also assess the
%DIFDELCMD < distributional
%DIFDELCMD < impacts of their interventions and target specific types of
%DIFDELCMD < commuters having high transport energy costs,
%DIFDELCMD < rather than treat areas as homogeneous blocks. This potential is
%DIFDELCMD < demonstrated with a case study of South
%DIFDELCMD < Yorkshire, where commuting energy use is cross-tabulated by
%DIFDELCMD < socio-economic variables and disaggregated over geographical space.
%DIFDELCMD < % The areas where commuting energy use
%DIFDELCMD < % is most unevenly distributed (in urban centres) will likely benefit most from
%DIFDELCMD < % policies that target
%DIFDELCMD < % the specific groups that have the greatest impact.
%DIFDELCMD < The areas where commuting energy use is less evenly distributed across the population, 
%DIFDELCMD < for example in urban centres, are
%DIFDELCMD < likely to benefit most from policies that target the specific groups.
%DIFDELCMD < Areas where commuter
%DIFDELCMD < energy use is more even, such as
%DIFDELCMD < Stocksbridge (in Northwest Sheffield), will benefit from more universal policies.
%DIFDELCMD < 

%DIFDELCMD < % The findings
%DIFDELCMD < % confirm the common-sense notion that those on high-incomes are more
%DIFDELCMD < % energy intensive in their travel to work behaviours and quantify by
%DIFDELCMD < % how much (the top income quintile uses around 3 times more energy
%DIFDELCMD < % for commuting than the bottom, although this varies from place to place).
%DIFDELCMD < % In terms of vulnerability, the research has identified deprived rural areas
%DIFDELCMD < % isolated from large employment centres as most at risk. Individual-level risk
%DIFDELCMD < % factors could include ease of relocating work more locally, opportunities
%DIFDELCMD < % for lift sharing or, in extreme cases, physical fitness and access to
%DIFDELCMD < % a suitable bicycle. None of these ideas are new, but the strength
%DIFDELCMD < % of evidence to support them is.
%DIFDELCMD < 

%DIFDELCMD < The thesis contributes to human knowledge new information about
%DIFDELCMD < the energy costs of commuting, its variability
%DIFDELCMD < at various levels and insight into the implications.
%DIFDELCMD < New methods of generating and analysing individual-level
%DIFDELCMD < data for the analysis of commuter energy use have also been developed.
%DIFDELCMD < % These methods are reproducible and should be of use to others % dad'd
%DIFDELCMD < % aiming to investigate the energy security, resource efficiency
%DIFDELCMD < % and long-term welfare benefits of interventions in personal travel systems.
%DIFDELCMD < These are reproducible and will be of interest
%DIFDELCMD < to researchers and policy makers investigating
%DIFDELCMD < the energy security, resource efficiency and potential welfare impacts
%DIFDELCMD < of interventions in personal travel systems.
%DIFDELCMD < }
%DIFDELCMD < %%%
...
For the first time this method is deployed to investigate variability in commuter energy
use both between and within small administrative zones.
% and
...
% \begin{itemize}
%  \item Parts of \cref{setsim} have been published, in \citep{Lovelace2013-trs}.
%  \item ``Spatial microsimulation in R'', a supplement to \citet{Lovelace2013-trs},
%  is based on \Cref{simplementing}.
%  \item The results presented in \cref{Chapter7} have been
%  published, in \citep{Lovelace2013-jtrg}.
% \end{itemize}
parts of \cref{setsim} appear in \citet{Lovelace2013-trs};
``Spatial microsimulation in R'', a supplement to \citet{Lovelace2013-trs},
 is based on \Cref{simplementing}; and the results presented in \cref{Chapter7} have
 also been published \citep{Lovelace2013-jtrg}.
...
information about the world. 
 %DIF < Expand and quote from Illich
%DIF > Expand and quote from Illich hh 

Because of the scale and complexity of the previously mentioned global
problems, it is \DIFdelbegin \DIFdel{temping }\DIFdelend \DIFaddbegin \DIFadd{tempting }\DIFaddend to focus solely on the detail of energy use in commuting
...
times faster than any natural change in Earth's past history\DIFdelbegin \DIFdel{with the exception
of the }\DIFdelend \DIFaddbegin \DIFadd{, except
...
provided by reason for \DIFdelbegin \DIFdel{trip}\DIFdelend \DIFaddbegin \DIFadd{trips}\DIFaddend , but estimates can be made by
...
\DIFdelbegin \DIFdel{On }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend addition to the impacts of climate change,
...
of understanding of their \DIFdelbegin \DIFdel{local-level }\DIFdelend \DIFaddbegin \DIFadd{local level }\DIFaddend manifestations and poor retention in memory:
although physical
...
get a \pounds900 bicycle through \DIFdelbegin \DIFdel{them}\DIFdelend \DIFaddbegin \DIFadd{it}\DIFaddend . That's the semi-solution that
...
sway for Chris, \DIFdelbegin \DIFdel{with the exception of the }\DIFdelend \DIFaddbegin \DIFadd{except for the }\DIFaddend location of his friends.
...
%DIF <  sections end. These national-level data, placed in context, help to introduce
%DIF >  sections end. These national level data, placed in context, help to introduce
...
the \DIFdelbegin \DIFdel{the }\DIFdelend most time-consuming reason for personal travel in the UK, accounting
...
is reflected in the \DIFdelbegin \DIFdel{the }\DIFdelend aims and objectives. These, which have
...
As outlined in \cref{Chapter1}, energy use in transport is \DIFdelbegin \DIFdel{bound-up }\DIFdelend \DIFaddbegin \DIFadd{bound up }\DIFaddend with a
...
expounded by some based on the strength of rhetoric, \DIFdelbegin \DIFdel{social-theory }\DIFdelend \DIFaddbegin \DIFadd{social theory }\DIFaddend and
...
characterising and 
modelling active travel patterns \DIFdelbegin \DIFdel{also}\DIFdelend \DIFaddbegin \DIFadd{as well}\DIFaddend . \citet{Millward2013} used GPS
...
as qualitative information about the reason for \DIFaddbegin \DIFadd{the }\DIFaddend trip.
...
and \DIFdelbegin \DIFdel{leveling-off }\DIFdelend \DIFaddbegin \DIFadd{levelling-off }\DIFaddend towards 0\%  after around 5 km for walking and 15 km for cycling. 
...
is generally not considered in \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend studies, although
...
on the factors related to \emph{\DIFdelbegin \DIFdel{aggregate-level}\DIFdelend \DIFaddbegin \DIFadd{aggregate level}\DIFaddend } variability in commuter flow
patterns. Local, regional and \DIFdelbegin \DIFdel{national-level }\DIFdelend \DIFaddbegin \DIFadd{national level }\DIFaddend studies are needed.
...
its indirect influence on behaviour\DIFdelbegin \DIFdel{, which is rarely direct}\DIFdelend . It is for this
...
commuting to the \DIFdelbegin \DIFdel{Tyrrel}\DIFdelend \DIFaddbegin \DIFadd{Tyrrell}\DIFaddend 's crisp factory would then become too expensive) and
...
\begin{itemize}
 ...
 study also combined \DIFdelbegin \DIFdel{household-level }\DIFdelend \DIFaddbegin \DIFadd{household level }\DIFaddend variables; it was found that a high number
...
\DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend logit model, with long or short distance commute
...
The \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend data allowed for the investigation of socio-demographic
...
In the context of the compact city debate, an \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend regression
...
}\DIFaddend returned to the question of the factors affecting commute time in US
...
Moreover, it is assumed that the former is a \DIFdelbegin \DIFdel{a }\DIFdelend close enough proxy of the latter
...
category, miscellaneous studies on transport \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend energy, will emphasise this
...
to researchers at least since the \DIFdelbegin \DIFdel{1800's }\DIFdelend \DIFaddbegin \DIFadd{1800s }\DIFaddend when \citet{tredgold1835practical}
...
\cref{fgabrielli}). This is a recurring \DIFdelbegin \DIFdel{them }\DIFdelend \DIFaddbegin \DIFadd{theme }\DIFaddend in \cref{Chapter5}.
...
\citet{Gabrielli1950} and \DIFdelbegin \DIFdel{its }\DIFdelend \DIFaddbegin \DIFadd{their }\DIFaddend successors made large advances in understandings
...
\DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy use studies.
Despite the activity of these research areas, 
there has been limited uptake of \DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy
...
\begin{itemize}
 ...
 direct emissions, to the detriment of understanding of the \DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend or
 ...
 \item Because of the focus on the \DIFdelbegin \DIFdel{national-level}\DIFdelend \DIFaddbegin \DIFadd{national level}\DIFaddend , papers in the area
 could be argued as offering little in the way of support to local and regional transport
 planners. This is an important oversight because local and \DIFdelbegin \DIFdel{regional-level
 }\DIFdelend \DIFaddbegin \DIFadd{regional level
 }\DIFaddend transport planners vastly outnumber national policy makers (in staff, if not
...
\DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy costs of different modes presented in the same chapter
...
commuting statistics. The energy \DIFdelbegin \DIFdel{costs }\DIFdelend \DIFaddbegin \DIFadd{cost }\DIFaddend of commuting is not a
...
infrastructure on energy use for commuting \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend determined. These are new
...
mainstream economics. \citet{Sexton2011} \DIFdelbegin \DIFdel{paper }\DIFdelend set out to
...
\citet{Sexton2011} the need to \DIFdelbegin \DIFdel{asses }\DIFdelend \DIFaddbegin \DIFadd{assess }\DIFaddend potential future impacts of high oil prices
...
\DIFdelbegin \DIFdel{past }\DIFdelend experience, concepts and intuition, while
...
law. The \DIFdelbegin \DIFdel{the }\DIFdelend law is falsifiable (and has been falsified on numerous occasions!)
...
people (`consumers') interacting with \DIFdelbegin \DIFdel{higher-level }\DIFdelend \DIFaddbegin \DIFadd{higher level }\DIFaddend `niches' and `regimes' to
...
to \DIFdelbegin \DIFdel{read-through }\DIFdelend \DIFaddbegin \DIFadd{read through }\DIFaddend long verbal descriptions or understand mathematical formulae.
...
\footnote{\DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend efficiency of electricity production, for example\DIFaddbegin \DIFadd{.}\DIFaddend }
through \DIFaddbegin \DIFadd{to
}\DIFaddend economic-thermodynamic definitions\footnote{For example, the efficiency of
...
of \DIFdelbegin \DIFdel{of }\DIFdelend freight transport \citep{Kamakate2009}. It has rarely been used to compare
...
economy and the natural environment \citep{Holling2001}. In \DIFaddbegin \DIFadd{the }\DIFaddend sustainability
literature, \DIFaddbegin \DIFadd{the }\DIFaddend term is rarely quantified (see \citealp{Bridge2010}). However,
...
straight line'' \citep[p. 73]{Newton1848}. In the context of transport \DIFdelbegin \DIFdel{system}\DIFdelend \DIFaddbegin \DIFadd{systems}\DIFaddend ,
...
study is quite new (\DIFdelbegin \DIFdel{with the exception of }\DIFdelend \DIFaddbegin \DIFadd{except for }\DIFaddend a flurry of work following the
...
This chapter reviews existing work that uses \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend data and modelling
...
microsimulation that is \DIFdelbegin \DIFdel{is }\DIFdelend central to the thesis. The method enables
\DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend and geographical variation in commuting behaviour
...
frequently used \DIFdelbegin \DIFdel{in }\DIFdelend throughout the thesis, so this chapter begins with
definitions.
...
are \DIFdelbegin \DIFdel{are }\DIFdelend grouped together under the broad term `urban modelling' and
...
signified a theoretical turn away from \DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend analyses and towards a
...
defined as
follows: the simulation of \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend variables within the geographic
...
\DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend transport modelling (\citealp{Pritchard2012}; \cref{f:msim-schematic}).
...
both \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend and geographically aggregated data
...
that are shared between the \DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend data and
...
census datasets are aggregated, whereas \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend data are generally
...
to some extent using \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend data allocated to geographical zones
...
prospect is unlikely to materialise in the UK in the \DIFdelbegin \DIFdel{short-term}\DIFdelend \DIFaddbegin \DIFadd{short term}\DIFaddend ,
...
changes \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the real spatial microdata \citep{Holm1987}.
...
purposes, to:
\begin{itemize}
 \item model variables whose spatial distribution at the \DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend is
otherwise unknown (e.g.~\citealp{Ballas1999}).
\item estimate the \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend distributions of variables within small
...
and thus the likely \DIFdelbegin \DIFdel{local-level }\DIFdelend \DIFaddbegin \DIFadd{local level }\DIFaddend effects of policy change \citep{Ballas2001}
...
variables' are the \DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend variables that link the zonal and individual
...
are \DIFdelbegin \DIFdel{are }\DIFdelend generated using integerisation.
...
data sets, methods of analysis and \DIFdelbegin \DIFdel{and }\DIFdelend conventions of mathematical notation that
...
early quantitative geographers had to \DIFdelbegin \DIFdel{make-do }\DIFdelend \DIFaddbegin \DIFadd{make do }\DIFaddend by
...
is that empirical data seldom \DIFdelbegin \DIFdel{fits }\DIFdelend \DIFaddbegin \DIFadd{fit }\DIFaddend into any neat model and therefore
\DIFdelbegin \DIFdel{distracts }\DIFdelend \DIFaddbegin \DIFadd{distract }\DIFaddend from explanation.
This point was made as early as the \DIFdelbegin \DIFdel{1970's}\DIFdelend \DIFaddbegin \DIFadd{1970s}\DIFaddend , accompanied by the
warning that the
...
\DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend behaviour depending on their zone of habitation can therefore
...
Walter Christaller's central place theory of the \DIFdelbegin \DIFdel{1930's }\DIFdelend \DIFaddbegin \DIFadd{1930s }\DIFaddend provided an integrated
...
advantage they seem to have had \DIFdelbegin \DIFdel{was }\DIFdelend a clear theoretical focus and this may
...
\DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend data now available could distract from explanation. As
...
}\DIFdelend car --- undoubtedly drove forward research seeking to understand and model
...
Originally proposed and formalised mathematically in the field \DIFaddbegin \DIFadd{of }\DIFaddend statistical
...
can be used to estimate the maximum likelihood of \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend attributes
...
\citet{Ballas1999} used IPF to allocate \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend survey data to
...
manipulating \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend variables based on estimated
...
\DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend methods for policy evaluation, he was more concerned with time than
he was with
space. %%%!!!referred
...
\DIFdelbegin \DIFdel{1960's }\DIFdelend \DIFaddbegin \DIFadd{1960s }\DIFaddend applications. This can partly be attributed to the
...
geography to \DIFdelbegin \DIFdel{the }\DIFdelend tackle the research challenge set out by
...
of \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend variables and projections of future change.
...
%DIF <  distribution of their individual-level attributes and how they were allocated to
%DIF >  distribution of their individual level attributes and how they were allocated to
% different
...
%DIF <  micro-level dataset containing all Swedish individuals is available to
%DIF >  micro level dataset containing all Swedish individuals is available to
...
interaction models and \DIFdelbegin \DIFdel{household-level }\DIFdelend \DIFaddbegin \DIFadd{household level }\DIFaddend phenomena, can be added to this basic
...
projected change in \DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend variables
(e.g.~\citealp{Ballas2005-ireland}).

\DIFdelbegin %DIFDELCMD < \begin{table}[h]
%DIFDELCMD < \centerline{}
%DIFDELCMD < \caption{Typology of spatial microsimulation methods}
%DIFDELCMD < \vspace{0.25 cm}
%DIFDELCMD < \footnotesize{
%DIFDELCMD < \begin{tabular}{p{1.6cm}p{2.4cm}p{3.5cm}p{3.0cm}p{2cm}}
%DIFDELCMD < \toprule
%DIFDELCMD < {Type} & {Reweighting technique} & {Pros} & {Cons} &
%DIFDELCMD < {Example} \\ \midrule
%DIFDELCMD < \multirow{3}{2cm}{\vspace{0.3cm} \\ Determ-  inistic\\\vspace{0.3cm}
%DIFDELCMD < Re-\\weighting} & Iterative proportional fitting (IPF) & Simple, fast, accurate,
%DIFDELCMD < avoids local optima and random numbers & Non-integer weights &
%DIFDELCMD < \citep{Tomintz2008}⁠⁠ \\ \cmidrule{2- 5}
%DIFDELCMD < & Integerised IPF & Builds on IPF, provides integer
%DIFDELCMD < weights & Integerisation reduces model fit & \citep{Ballas2005c}⁠ \\
%DIFDELCMD < \cmidrule{2- 5}
%DIFDELCMD < &  GREGWT, generalised reweighting
%DIFDELCMD < & Fast, accurate,
%DIFDELCMD < avoids local optima and random numbers & Non-integer weights  &
%DIFDELCMD < \citep{Miranti2010}⁠ \\ \midrule
%DIFDELCMD < \multirow{2}{2cm}{ \\ Probab- ilistic  Combin-
%DIFDELCMD < atorial optim-
%DIFDELCMD < isation} & Hill climbing approach & The simplest solution to a combinatorial
%DIFDELCMD < optimisation, integer results & Can get stuck in local optima, slow &
%DIFDELCMD < \citep{Williamson1998}⁠ \\ \cmidrule{2-5}
%DIFDELCMD < & Simulated annealing & Avoids local minima, widely
%DIFDELCMD < used, multi-level constraints & Computationally intensive
%DIFDELCMD < & \citep{kavroudakis2012}⁠  \\ \midrule
%DIFDELCMD < \multirow{2}{2cm}{\vspace{0.3cm} \\ Dynamic} & Monte Carlo
%DIFDELCMD < randomisation to simulate ageing  & Realistic treatement of stochastic
%DIFDELCMD < life events such as death & Depends on accurate estimates of life event
%DIFDELCMD < probabilities & \citep{Vidyattama2010}⁠ \\
%DIFDELCMD < \cmidrule{2- 5}
%DIFDELCMD < & Implicitly dynamic & Simplicity, low
%DIFDELCMD < computational demands & Crude, must project constraint
%DIFDELCMD < variables & \citep{Ballas2005b}⁠ \\ \bottomrule
%DIFDELCMD < \end{tabular}
%DIFDELCMD < }
%DIFDELCMD < \label{typology}
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
...
weights & Integerisation reduces model fit & \mbox{%DIFAUXCMD
\citep{Ballas2005c}
}%DIFAUXCMD
⁠ \\
\cmidrule{2- 5}
...
\begin{itemize}
 ...
 \item reweight the \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend data to create a spatial microdataset
...
individual: ``it can control either for \DIFdelbegin \DIFdel{agent-level or for group-level
}\DIFdelend \DIFaddbegin \DIFadd{agent level or for group level
}\DIFaddend attributes but not for both simultaneously'' \citep[p.~5]{Muller2010}.
...
attributes (age, \DIFdelbegin \DIFdel{eduction}\DIFdelend \DIFaddbegin \DIFadd{level of education}\DIFaddend ). These results offer the promise of allowing
\DIFdelbegin \DIFdel{family-level }\DIFdelend \DIFaddbegin \DIFadd{family level }\DIFaddend microdata generation from deterministic reweighting algorithms
such as IPF.
...
strategic \DIFdelbegin \DIFdel{planing }\DIFdelend \DIFaddbegin \DIFadd{planning }\DIFaddend \citep{Wilson1998-past} and hence large contracts.
...
\label{s:urbanmodel}
Urban modelling goes beyond the estimation of \DIFdelbegin \DIFdel{individual-level
}\DIFdelend \DIFaddbegin \DIFadd{individual level
}\DIFaddend characteristics, as performed in spatial microsimulation.
...
was carefully designed from the \DIFdelbegin \DIFdel{ground-up }\DIFdelend \DIFaddbegin \DIFadd{ground up }\DIFaddend to be the most powerful, user-friendly
...
transportation \DIFdelbegin \DIFdel{microsimualtion }\DIFdelend \DIFaddbegin \DIFadd{microsimulation }\DIFaddend systems such as ILUTE ... ILUMASS
...
models based on \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend data:
...
\citep[p.~15]{timmermans2003saga}. Thus ILUTE can \DIFdelbegin \DIFdel{can }\DIFdelend be used to analyse a wide range
...
%\emph{TRESIS}
\emph{UrbanSim}, like ILUTE, is a \DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend integrated land-use transport
...
it has been used for \DIFaddbegin \DIFadd{a }\DIFaddend range of applications including as a tool
...
include a \DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend population synthesizer, a trip generator, route planner
...
providing quantitative geographers and others with \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend data, into a more
...
and developing the theory behind \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend behaviour for even longer
\citep{Wilson1970}.
The novel
...
existing \DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend transport models such as
MATSim (\cref{Chapter3}). However, these models focus on transport:
\DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend attributes provide an optional add-on.
The methods presented in this thesis operate the other
way around: \DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend characteristics generated by spatial microsimulation
...
\DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend analyses are described in general terms in the previous
...
in \cref{senergyusedata}, as good energy data \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend vital to
...
Social survey data \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend made available both as geographically aggregated
counts from the census (\cref{sgeoaggdata}) and more detailed \DIFdelbegin \DIFdel{individual-level
}\DIFdelend \DIFaddbegin \DIFadd{individual level
}\DIFaddend variables from  nationwide surveys which take a representative sample of
...
little sense of \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend variation. This can be supplemented by datasets
...
level. The National Travel Survey (NTS) is the other \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend dataset
used. It is
...
benefits, the data \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend temporal: it would allow changes in the geographical
...
\%). This \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{dataset }\DIFaddend is from 2010 and can be found in Table NTS0402 from
...
methods of translating the total \DIFdelbegin \DIFdel{in }\DIFdelend into commuter energy use only could be used,
...
is the national census, which must be answered by every household. The \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{dataset }\DIFaddend is
released a year or so after
each census, which has taken place every 10 years (\DIFdelbegin \DIFdel{with the exception of }\DIFdelend \DIFaddbegin \DIFadd{except }\DIFaddend 1941)
...
This \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{dataset }\DIFaddend is provided for a 10\% sample before 2001, which is
problematic in small areas.\footnote{The \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{dataset }\DIFaddend is provided down to enumeration
...
aggregation}
area can lead to long processing times, messy maps and \DIFdelbegin \DIFdel{overcomplexity}\DIFdelend \DIFaddbegin \DIFadd{over-complexity}\DIFaddend .
...
%DIFDELCMD < \includegraphics[width=14cm]{scales-map}
%DIFDELCMD < \caption[National, regional and city-wide scales of analysis]{National, regional
%DIFDELCMD < and city-wide scales of analysis, as illustrated
%DIFDELCMD < by a range of administrative boundaries. Yorkshire and the Humber (YatH, left),
%DIFDELCMD < South Yorkshire (top right) and Sheffield (bottom right)
%DIFDELCMD < are the study areas used for this section.}
%DIFDELCMD < \label{f:scales}
...
\DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend variables, regardless of the scale of analysis. It is
still important to choose an appropriate scale, as lower
geographies will provide more localised information, yet be \DIFdelbegin \DIFdel{be harder
more difficult }\DIFdelend \DIFaddbegin \DIFadd{harder
}\DIFaddend to analyse and visualise. 
...
Cross-tabulated count data refers to categories which are \DIFdelbegin \DIFdel{split-up }\DIFdelend \DIFaddbegin \DIFadd{split up }\DIFaddend into
...
sharing, links between car ownership and employment, and \DIFdelbegin \DIFdel{household-level
}\DIFdelend \DIFaddbegin \DIFadd{household level
}\DIFaddend microsimulation.
...
To perform any kind of microsimulation study, a \DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend dataset must
...
Table \ref{t:indata} illustrates some important \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend `target
...
\DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend variables related to commuting. The USd is the most
...
%DIFDELCMD < \caption[Selected individual-level variables related to commuting]{Selected
%DIFDELCMD < individual-level variables related to commuting,
%DIFDELCMD < available from the Understanding Society dataset.}
%DIFDELCMD < \begin{tabular}{|p{2.5cm}|p{2.5cm}|p{4cm}|p{4cm}|}
%DIFDELCMD < \hline
%DIFDELCMD < Attribute & Variable & Measurement & Comment \\ \hline
%DIFDELCMD < Type of car & Household variable 146 & Engine size of cars: \hspace{1cm} $< 1.4
%DIFDELCMD < ,
%DIFDELCMD < 1.4-1.9 $, or $\ge 2 l$  & Data on additional cars also available \\
%DIFDELCMD < \hline
%DIFDELCMD < Household income & Household variable 193 & Net household income,
%DIFDELCMD < \pounds/month & Equivalised income must be calculated \\ \hline
%DIFDELCMD < % Car sharing potential & indresp: envhabit10 & Frequency of car sharing: 5
%DIFDELCMD < point
%DIFDELCMD < % scale from “always” to “never” & Subjective \\ \hline
%DIFDELCMD < Telecommuting potential & Individual-level variable 953 & 7 point scale from “no
%DIFDELCMD < access”
%DIFDELCMD < to “everyday” & Must be linked with type of work \\ \hline
%DIFDELCMD < Ease of moving home & Household variable 171 & Number of children (aged 15 or
%DIFDELCMD < under) in household   & One indication of how settled household is \\ \hline
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \label{t:indata}
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
...
National Travel Survey (NTS). This household and \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend survey was
...
The \DIFdelbegin \DIFdel{household-level data }\DIFdelend \DIFaddbegin \DIFadd{household level dataset }\DIFaddend is most useful at providing insight into people's
...
At the \DIFdelbegin \DIFdel{individual-level}\DIFdelend \DIFaddbegin \DIFadd{individual level}\DIFaddend , the NTS also provides a range of useful
...
\DIFdelbegin \DIFdel{household-level }\DIFdelend \DIFaddbegin \DIFadd{household level }\DIFaddend dataset, tenancy, household income (in three bands), social
...
The \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend dataset consists of 175 variables which contain more
...
As with the \DIFdelbegin \DIFdel{household-level }\DIFdelend \DIFaddbegin \DIFadd{household level }\DIFaddend variables, the main utility of these is adding
...
mentioned so far deal with travel to work directly, however. Commuting data \DIFdelbegin \DIFdel{is
}\DIFdelend \DIFaddbegin \DIFadd{are
}\DIFaddend provided by variable I180 (``usual means of travel to work'') and I92 (``work
...
An \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend ``distance to work'' variable can be calculated based on
the
%%% was it??? or no???
...
\DIFdelbegin \DIFdel{Travel to work data is usually a gross oversimplification }\DIFdelend \DIFaddbegin \DIFadd{Quantitative information about travel to work usually oversimplifies }\DIFaddend of
...
\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD < \includegraphics[width=14cm]{freq-nts}
%DIFDELCMD < \caption[Bar-plot of frequency of working from home]{Bar-plot of frequency of
%DIFDELCMD < working from home (wfh) in the NTS, 2002-2008. Note: only 7\% of the
%DIFDELCMD < individual-level sample answered this question; around half of the
%DIFDELCMD < non-respondents do not work.}
%DIFDELCMD < \label{ffreq-nts}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}
...
have a lot of NA values, and are very patchy\DIFdelbegin \DIFdel{)}\DIFdelend . This would allow an
additional constraint variable to be used for \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend NTS data as an
...
On the other extreme, many geo-referenced \DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend datasets on
commuting behaviour have been collected. These are generally small in geographical
coverage (at
least relative to the nationwide \DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend commuting datasets
...
to each individual participating in the survey, although this \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{dataset }\DIFaddend is generally
...
travel survey as the basis for assessing the energy impacts of \DIFdelbegin \DIFdel{organisation-level
}\DIFdelend \DIFaddbegin \DIFadd{organisation level
}\DIFaddend change. This did not materialise in part due to time constraints and in part
due to concern over access to the geo-referenced \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend data.
}
A very large and detailed example of \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend geo-referenced \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend dataset
...
these methods). However, the methods of analysis used to interpret \DIFdelbegin \DIFdel{these
data }\DIFdelend \DIFaddbegin \DIFadd{the
datasets }\DIFaddend presented in the latter sections of this chapter and in \cref{s:workdes}
\DIFdelbegin \DIFdel{could-well }\DIFdelend \DIFaddbegin \DIFadd{could well }\DIFaddend be applicable to these valuable \DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend datasets.
...
behaviour, \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend sufficient to calculate the energy costs of commuting at
individual and aggregate levels. The scope of this work extends beyond mere
description, however. Additional input \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{information }\DIFaddend is required to explain \emph{why}
...
and
remoteness.
%DIF < %% May want to say something about additional individual-level vars too.
%DIF > %% May want to say something about additional individual level vars too.
...
work to do the shopping or on the way there to `\DIFdelbegin \DIFdel{drop-off }\DIFdelend \DIFaddbegin \DIFadd{drop off }\DIFaddend the kids'), habitual
...
pre-defined \DIFdelbegin \DIFdel{path}\DIFdelend \DIFaddbegin \DIFadd{paths}\DIFaddend . Thus, although trains (and to a limited extent buses, when
...
was found that \DIFdelbegin \DIFdel{OSM data is }\DIFdelend \DIFaddbegin \DIFadd{datasets derived from OSM are }\DIFaddend generally accurate, especially for large
...
The final point to consider is usability. While OSM \DIFdelbegin \DIFdel{data is }\DIFdelend \DIFaddbegin \DIFadd{datasets are }\DIFaddend available
worldwide, it is not the standard dataset in use by local planning departments,
which generally have institutional access to Ordnance Survey data. \DIFdelbegin \DIFdel{OSM data }\DIFdelend \DIFaddbegin \DIFadd{The 
OSM data source }\DIFaddend is generally also more difficult for non-expert users to find and
download.\footnote{The
OSM transport \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{dataset }\DIFaddend presented in \cref{fosm-trans}, for example, was not
...
(OSM \DIFdelbegin \DIFdel{data }\DIFdelend is well suited to use in geo-databases such as PostGIS.)} Therefore,
...
useful for \DIFdelbegin \DIFdel{low-level }\DIFdelend \DIFaddbegin \DIFadd{low level }\DIFaddend analysis of likely routes of non-motorised modes. However, the
...
Topographic \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{datasets }\DIFaddend for the UK \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend available from the \DIFdelbegin \DIFdel{the }\DIFdelend following sources,
ranging from the coarsest to the finest:
\begin{itemize}
\item The Advanced Spaceborne Thermal Emission and Reflection Radiometer
(ASTER) sensor mounted on the Space Shuttle has produced \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{a dataset }\DIFaddend that has been
...
{http://www.jspacesystems.or.jp}. \DIFdelbegin \DIFdel{Digital }\DIFdelend \DIFaddbegin \DIFadd{A digital }\DIFaddend elevation \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{dataset }\DIFaddend was successfully
...
ranging) data for the majority of the island of Great Britain. \DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend data can be
...
%DIF <  Some individual-level data can be inferred  based on aggregate-level data.
%DIF >  Some individual level data can be inferred  based on aggregate level data.
...
%DIF <  Spatial microsimulation performs such inference for individual-level variables
%DIF >  Spatial microsimulation performs such inference for individual level variables
...
commuting: \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend data allocated to spatial areas. This is where
spatial microsimulation comes in, to combine the \DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend commuting
data with the \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend data presented in the previous section.
...
representative of the area (relative to the \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend dataset) the
...
\item in addition to enabling statistical operations, \DIFdelbegin \DIFdel{it�}\DIFdelend \DIFaddbegin \DIFadd{it'}\DIFaddend s a general programming
...
\begin{itemize}
 ...
 \DIFdelbegin \DIFdel{reference }\DIFdelend \DIFaddbegin \DIFadd{references }\DIFaddend \citep{wickham2011ggplot2, chang2012r}.
...
\citealp{Johnston1993, blien1998entropy}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
for a hypothetical area, as it would be \DIFdelbegin \DIFdel{download }\DIFdelend \DIFaddbegin \DIFadd{downloaded }\DIFaddend from census dissemination
...
\DIFdelbegin %DIFDELCMD < \begin{table}[htbp]
%DIFDELCMD < \centering
%DIFDELCMD < \caption{Reweighting the hypothetical microdata set in order to fit
%DIFDELCMD < Table \ref{t:s}.}
%DIFDELCMD < \begin{tabular}{lllll}
%DIFDELCMD < \toprule
%DIFDELCMD < {Individual} & {Sex} & {age-group} & {Weight} &
%DIFDELCMD < {New weight, w(2)} \\ \midrule
%DIFDELCMD < 1 & Male & Over-50 & 1 & $1 \times 4/3 = \frac{4}{3}$ \\
%DIFDELCMD < 2 & Male & Over-50 & 1 & $1 \times 4/3 = \frac{4}{3}$ \\
%DIFDELCMD < 3 & Male & Under-50 & 1 & $\textbf{1} \times
%DIFDELCMD < \textbf{8}/\textbf{2} = 4$ \\
%DIFDELCMD < 4 & Female & Over-50 & 1 & $1 \times 4/3 = \frac{4}{3}$ \\
%DIFDELCMD < 5 & Female & Under-50 & 1 & $1 \times 8/2 = 4$ \\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \label{t:new-weights}
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{table}[htbp]
\centering
...
After the \DIFdelbegin \DIFdel{individual-level data has }\DIFdelend \DIFaddbegin \DIFadd{individual level data have }\DIFaddend been re-aggregated (\cref{t:m2}),
...
distribution of \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend variables about which only
...
the \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{dataset }\DIFaddend is read line by line, following the
...
the \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend data will be referred to as USd (Understanding Society dataset)
and the geographic data as all.msim (for all constraint variables).
The code to read-in the \DIFdelbegin \DIFdel{individual-level data is }\DIFdelend \DIFaddbegin \DIFadd{individual level data are }\DIFaddend presented in code sample \ref{cusd}.
When called, the data \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend then displayed as a table (see listing \ref{cout}).
\DIFdelbegin %DIFDELCMD < \begin{lstlisting}[float=h, caption={Manual input of individual-level data
%DIFDELCMD < in R}, label=cusd]
%DIFDELCMD < # Read in the data in long form (normaly read.table() used)
%DIFDELCMD < c.names <- c("id", "age", "sex")
%DIFDELCMD < USd <- c(       1, 59, "m",
%DIFDELCMD <                 2, 54, "m",
%DIFDELCMD <                 3, 35, "m",
%DIFDELCMD <                 4, 73, "f",
%DIFDELCMD <                 5, 49, "f")
%DIFDELCMD < USd <- matrix(USd, nrow = 5, byrow = T) # Long data into matrix
%DIFDELCMD < USd <- data.frame(USd) # Convert this into a dataframe
%DIFDELCMD < names(USd) <- c.names # Add correct column names
%DIFDELCMD < USd$age <- as.numeric(levels(USd$age)[USd$age]) # Age is a numeric
%DIFDELCMD < \end{lstlisting}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{lstlisting}[float=h, caption={Manual input of individual level data
in R}, label=cusd]
...
\end{lstlisting}
\DIFaddend \begin{lstlisting}[float=h, caption={Output of the USd data frame}, label=cout]
...
position to start comparing how well our \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend survey dataset
...
results in a fit that is virtually perfect \DIFaddbegin \DIFadd{(}\DIFaddend \cref{fexits}\DIFaddbegin \DIFadd{)}\DIFaddend . More iterations
...
vulnerability metrics based on the \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend output from the spatial
...
quantitative analysis. They ensure that the data \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend properly
...
sophisticated plots (see \cref{fig:IPF-4c}\DIFdelbegin \DIFdel{, above}\DIFdelend ).
...
provides estimates of net and gross household incomes \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend the MSOA level. For the purposes of
...
confidential data\DIFdelbegin \DIFdel{.'' }\DIFdelend \DIFaddbegin \DIFadd{'' (this phrase appears in many of Nomis's tables.
One example can be found here:
}\href{http://www.nomisweb.co.uk/livelinks/4652.xls}{http://www.nomisweb.co.uk/livelinks/4652.xls}\DIFadd{).
}\DIFaddend 
...
data \DIFdelbegin \DIFdel{is}\DIFdelend \DIFaddbegin \DIFadd{are}\DIFaddend . How much have Nomis data been randomised, and in what way? 
...
suggest Nomis data \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend faithful to the original census results:
...
to \DIFdelbegin \DIFdel{to }\DIFdelend a fixed randomising factor.  To test this, the errors were plotted for
...
\begin{itemize}
 ...
 \item Aggregating local model outputs to \DIFdelbegin \DIFdel{a }\DIFdelend coarser geographical levels at which
 cross-tabulated data \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend available.
...
at the individual level \DIFdelbegin \DIFdel{also}\DIFdelend \DIFaddbegin \DIFadd{as well}\DIFaddend .
...
%DIF <  discussion of how best to use the individual-level data that is available
%DIF >  discussion of how best to use the individual level data that is available
% for model evaluation...
Because simulation, almost by definition, estimates something that is not
otherwise \DIFdelbegin \DIFdel{know}\DIFdelend \DIFaddbegin \DIFadd{known}\DIFaddend , it is hard to find reliable \DIFdelbegin \DIFdel{individual-level
}\DIFdelend \DIFaddbegin \DIFadd{individual level
}\DIFaddend data against which the estimates can be evaluated. For this reason
\DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend surveys could be conducted in a specific area where
...
\DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend data from one source to aggregated data from another.
...
characteristics are not recorded at the local level. \DIFdelbegin \DIFdel{Intergerisation }\DIFdelend \DIFaddbegin \DIFadd{Integerisation }\DIFaddend translates
...
To test each integerisation method, IPF was used to \DIFdelbegin \DIFdel{to }\DIFdelend generate an
array of weights that fit \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend survey data to
...
The technique successfully \DIFdelbegin \DIFdel{tops-up }\DIFdelend \DIFaddbegin \DIFadd{tops up }\DIFaddend integer populations yet has
...
provided in the Supplementary Information, other are encouraged to use, \DIFdelbegin \DIFdel{build-on
}\DIFdelend \DIFaddbegin \DIFadd{build on
}\DIFaddend and improve TRS integerisation.

A broader issue raised by \DIFdelbegin \DIFdel{the }\DIFdelend this research, that requires further
...
%DIF <  individual-level attributes such as mode (including type of car ownership),
%DIF >  individual level attributes such as mode (including type of car ownership),
...
%DIF <  zones via an individual-level `zone' variable, rather than being placed
%DIF >  zones via an individual level `zone' variable, rather than being placed
...
%DIF <  hundreds of individuals for each zone under investigation. For micro-level
%DIF >  hundreds of individuals for each zone under investigation. For micro level
...
% this purpose.}
%DIF <  \item Save the results in an individual-level variable.
%DIF >  \item Save the results in an individual level variable.
% \end{itemize}
...
life all affect the \DIFdelbegin \DIFdel{the }\DIFdelend commuter patterns people follow. The same is true of
...
are taken into \DIFdelbegin \DIFdel{accout}\DIFdelend \DIFaddbegin \DIFadd{account}\DIFaddend , let alone externalities such as pollution,
bureaucracy and disposal \citep{Perman2003}.
The costs of personal transport can be divided into variable and
fixed costs, which in turn are sub-divided \DIFaddbegin \DIFadd{(}\DIFaddend \cref{fcar-patterned}\DIFaddbegin \DIFadd{)}\DIFaddend .
...
A systematic method for analysing \DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy costs is
...
fleet and roads are different \DIFdelbegin \DIFdel{than }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend those of the USA, where things are larger.
Therefore the \DIFdelbegin \DIFdel{Fel's data is kept as a reserve }\DIFdelend \DIFaddbegin \DIFadd{numbers presented in \mbox{%DIFAUXCMD
\citet{Fels1975}
}%DIFAUXCMD
are
used only }\DIFaddend for comparison with more recent energy use data.
...
`patchwork' of results, with a different source for each mode \DIFaddbegin \DIFadd{(}\DIFaddend \cref{tmodes}\DIFaddbegin \DIFadd{)}\DIFaddend .
...
take them into \DIFaddbegin \DIFadd{account }\DIFaddend using sensitivity analysis. %!!! DB says incorporate into PhD!
...
Although \DIFaddbegin \DIFadd{the }\DIFaddend Defra dataset is primarily concerned with greenhouse gas
...
energy costs can be \DIFdelbegin \DIFdel{broken-down }\DIFdelend \DIFaddbegin \DIFadd{broken down }\DIFaddend to the level of
...
Society dataset (USd), the primary source of \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend microdata in this project,
...
}\DIFaddend (presented in \cref{tmodes}) and our own estimates calculated through \cref{eqmjco2}
...
\section{Calculating \DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy use} \label{ssystemlevel}
...
For the purposes of simplicity, it can be assumed that the \DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy
...
The first paper that formalised this problem in the context of \DIFdelbegin \DIFdel{system-level
...
}\DIFaddend , so the calculation of
...
\DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy costs for single trips to work and back ($ETsys$):
...
\DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy costs is rarely undertaken, and merits further comment
before
discussing the data that enable \DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy costs to be estimated. 

Fels' framework for calculating \DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy costs has been available
...
This reluctance to engage with \DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy
...
\DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy costs, but this is only useful if the input data is
...
%DIFDELCMD <  \includegraphics[width=14 cm]{screengrab-final-e}
%DIFDELCMD < \end{center}
%DIFDELCMD <  % cc-trans.png: 1113x529 pixel, 72dpi, 39.26x18.66 cm, bb=0 0 1113 529
%DIFDELCMD <  \caption{Screen shot of the spreadsheet used to calculate system-level energy
%DIFDELCMD <  costs.}
%DIFDELCMD <  \label{screengrab-final-e}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
...
only to petrol and diesel, but to electricity and food \DIFdelbegin \DIFdel{also}\DIFdelend \DIFaddbegin \DIFadd{as well}\DIFaddend . It is all \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{too }\DIFaddend easy
...
personal travel, which it is \DIFdelbegin \DIFdel{do }\DIFdelend not in this thesis),
% \footnote{} % !!! How many?
...
\DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy cost calculations: to produce 1 kWh (3.6 MJ) of electricity
...
even during the \DIFdelbegin \DIFdel{course of the }\DIFdelend day, as (generally inefficient) plants come
...
transportation on the overall \DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy costs of transport to work.
...
megajoules, they can be used as inputs the \DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy cost
...
do we select the estimate that seems: \DIFdelbegin \DIFdel{most authoritative? most }\DIFdelend \DIFaddbegin \DIFadd{Most authoritative? Most }\DIFaddend recent?
most related to the UK car fleet? \DIFdelbegin \DIFdel{do }\DIFdelend \DIFaddbegin \DIFadd{Do }\DIFaddend we use this as the basis for best
and worst-case scenarios? \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{Or }\DIFaddend do we take some kind of average?
...
only really \DIFdelbegin \DIFdel{know the value }\DIFdelend \DIFaddbegin \DIFadd{known }\DIFaddend to one significant figure. In place of using an
...
\DIFdelbegin \DIFdel{also}\DIFdelend \DIFaddbegin \DIFadd{as well}\DIFaddend .
...
etc.~that cars can \DIFdelbegin \DIFdel{can }\DIFdelend move people. All of these objects require large one-off
...
%DIF <  they are constrained by census statistics. Re-aggregating the individual-level
%DIF >  they are constrained by census statistics. Re-aggregating the individual level
...
\DIFaddbegin \DIFadd{(}\DIFaddend \cref{fn-trips-hist-prop}\DIFaddbegin \DIFadd{)}\DIFaddend , which reduces the annual energy use estimates.
...
To take this effect into account at the \DIFdelbegin \DIFdel{individual-level}\DIFdelend \DIFaddbegin \DIFadd{individual level}\DIFaddend , a simple
regression model was \DIFdelbegin \DIFdel{ran }\DIFdelend \DIFaddbegin \DIFadd{run }\DIFaddend to find the relationship between average trip
...
Euclidean distance (\cref{tdistable}). For \DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend calculations,
...
in hotels \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend restaurants are closer to 4 hours, and the fact that some people
...
cars \DIFdelbegin \DIFdel{on a few occasions}\DIFdelend \DIFaddbegin \DIFadd{occasionally}\DIFaddend , resulting in the conclusion that empty seats
...
\citep{bouwman2000tracking}. Using a \DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend model written in
...
the \DIFdelbegin \DIFdel{the }\DIFdelend estimates. Therefore, taking these issues into account, it was decided not
...
depending on the \DIFdelbegin \DIFdel{the }\DIFdelend local road network and proximity of impassable obstacles
...
inter-modal \DIFdelbegin \DIFdel{variation }\DIFdelend \DIFaddbegin \DIFadd{variations }\DIFaddend in circuity: pedestrians
...
values that must be displayed by law on all car adverts \DIFaddbegin \DIFadd{(}\DIFaddend \cref{f:testcycle}\DIFaddbegin \DIFadd{)}\DIFaddend :
two efficiencies are calculated --- urban and extra-urban. According to
\citet{Pelkmans2006}, urban driving \DIFaddbegin \DIFadd{uses }\DIFaddend around 30\% more energy per unit distance
...
cycles \DIFaddbegin \DIFadd{(}\DIFaddend \cref{f:testcycle}\DIFaddbegin \DIFadd{)}\DIFaddend . The efficiency impact ranges from a
34\% increase for the
...
spatial microdata. That is not \DIFdelbegin \DIFdel{take-away }\DIFdelend \DIFaddbegin \DIFadd{to take away }\DIFaddend from the importance of driver
...
a low powered and energy efficient car}\DIFaddend .
...
The previous section illustrates that fuel economy should not be seen simply as \DIFdelbegin \DIFdel{as
}\DIFdelend \DIFaddbegin \DIFadd{a
}\DIFaddend fixed number, such as 3 MJ/km for cars. Even at the aggregate level, the
...
one \DIFdelbegin \DIFdel{modes }\DIFdelend \DIFaddbegin \DIFadd{mode }\DIFaddend whilst leaving the others unchanged would not be consistent.
...
%DIFDELCMD <   \centerline{
%DIFDELCMD <     \includegraphics[width = 14 cm]{etime.png}}
%DIFDELCMD <     \rule{35em}{0.5pt}
%DIFDELCMD <   \caption[Fleet efficiency of UK vehicles over time]{Energy consumption of new
%DIFDELCMD < cars, the entire car fleet, and the energy intensity of road road passengers
%DIFDELCMD < transport kilometre over time. Data: \citep{Decc2011t}.}
%DIFDELCMD <   \label{fig:etime}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
...
transport kilometre over time. Data: \mbox{%DIFAUXCMD
\citep{Decc2011t}
}%DIFAUXCMD
.}
  \label{fig:etime}
...
per year between \DIFdelbegin \DIFdel{in }\DIFdelend the decade following 1973, before levelling out during the
1980s.


\DIFdelbegin %DIFDELCMD < \begin{figure}[h]
%DIFDELCMD <   \centerline{
%DIFDELCMD <     \includegraphics[width = 14 cm]{natefic}}
%DIFDELCMD <     \rule{35em}{0.5pt}
%DIFDELCMD <   \caption[Fleet efficiencies of new cars in the UK and USA, 1977-2012]
%DIFDELCMD <   {Fleet efficiencies of new cars in the UK and USA, 1977-2012.
%DIFDELCMD <   Data calculated from 
%DIFDELCMD <   \citet{Schipper1993} (UK1, USA), assuming an energy content of fuel of
%DIFDELCMD <   32 MJ/l, and the Department for travel
%DIFDELCMD <   \citep[table VEH0256]{Df2013licencing}, assuming a conversion factor
%DIFDELCMD <   of 14.4 between kg of CO$_2$ and MJ.}
%DIFDELCMD <   \label{fnatefic}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}[h]
  \centerline{
    ...
    \rule{35em}{0.5pt}
  \caption{Comparison of UK car fleet efficiency estimates over time \mbox{%DIFAUXCMD
\citep{Df2013licencing}
}%DIFAUXCMD
.}
  \label{fnatefficall}
\end{figure}
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[h]
%DIFDELCMD <   \centerline{
%DIFDELCMD <     \includegraphics[width = 14 cm]{natefficall}}
%DIFDELCMD <     \rule{35em}{0.5pt}
%DIFDELCMD <   \caption{Comparison of fleet efficiency estimates over time.}
%DIFDELCMD <   \label{fnatefficall}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}[h]
  \centerline{
...
(UK1, USA), assuming an energy content of fuel of
  32 MJ/l, and the Department for travel
  \mbox{%DIFAUXCMD
\citep[table VEH0256]{Df2013licencing}
}%DIFAUXCMD
...
}\DIFaddend in \cref{sdirecte}), combined with the difficulty of `measuring'
energy economy in
practice
...
at the beginning of the century, with almost all \DIFdelbegin \DIFdel{of }\DIFdelend the growth attributable to
...
UK's work travel \DIFaddbegin \DIFadd{(}\DIFaddend \cref{fturnmode}\DIFaddbegin \DIFadd{)}\DIFaddend , but this has not happened uniformly over
...
commuting than all other commuter modes put together \DIFaddbegin \DIFadd{(}\DIFaddend \cref{f1981}\DIFaddbegin \DIFadd{)}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \begin{figure}[htbp]
%DIFDELCMD <   \centerline{
%DIFDELCMD <     \includegraphics[width=16cm]{1981-2001}}
%DIFDELCMD <   \caption[The growing dominance of the car, 1981 to 2001]
%DIFDELCMD <   {Areas in which driving to work accounts for more than half of all
%DIFDELCMD <   commuter trips in Yorkshire and the Humber. Ward-level data from
%DIFDELCMD <   Casweb.} %%% could update with methods
%DIFDELCMD <   \label{f1981}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
...
is \DIFdelbegin \DIFdel{is }\DIFdelend difficult and energy intensive to store --- due to high pressure and low temperature
...
\begin{itemize}
 ...
 \item Additional strain on \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend ailing electricity grid \citep{dyke2010impact,
 webster1999can}.
...
power supply implies increased \DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy costs.
...
models were \DIFdelbegin \DIFdel{rolled-out }\DIFdelend \DIFaddbegin \DIFadd{rolled out }\DIFaddend alongside policies to reduce speed, increase occupancy
...
commercial viability by approaching the problem from the `\DIFdelbegin \DIFdel{top-down}\DIFdelend \DIFaddbegin \DIFadd{top down}\DIFaddend '. This
...
}\DIFaddend depends on the aims of the research:
...
homogeneity does not \DIFdelbegin \DIFdel{not }\DIFdelend translate into reality, as regional bus operators,
...
available and geographically disaggregated data on \DIFdelbegin \DIFdel{on }\DIFdelend the matter is lacking for
...
accounts for
% more than 95\% %old
the vast majority of \DIFdelbegin \DIFdel{of }\DIFdelend the energy costs of personal travel. 
...
less \DIFdelbegin \DIFdel{less }\DIFdelend is known, such as
...
fleet efficiency. Caution should be \DIFdelbegin \DIFdel{be }\DIFdelend used when interpreting the results,
acknowledging the fact that the 2002 data is much more sparse on
emissions estimates because the \DIFdelbegin \DIFdel{the }\DIFdelend majority of cars in the vehicle fleet
...
The raw
data on which \DIFdelbegin \DIFdel{MSOA-level }\DIFdelend \DIFaddbegin \DIFadd{MSOA level }\DIFaddend fleet efficiency estimates are based was provided by
...
\DIFdelbegin \DIFdel{YatH }\DIFdelend \DIFaddbegin \DIFadd{Yorkshire and the Humber }\DIFaddend was taken. These calculations suggest the region's car fleet is more
...
fuel requirements of cars and bicycles for direct and \DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend fuel use
...
in \cref{sdirecte}), and three indirect components of \DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy
...
study, over 20 years ago \DIFaddbegin \DIFadd{(}\DIFaddend \cref{fmjpkm1991}\DIFaddbegin \DIFadd{)}\DIFaddend .
...
to calculate energy costs by trip. Although the larger \DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy costs
...
because the direct energy costs are more certain, come \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend official data sources and
...
\DIFdelbegin \DIFdel{reduction
}\DIFdelend in emissions, 20\% of final energy delivered from renewable sources and
a 20\% increase in energy efficiency.
}
Longer-term scenarios 
are less constrained by such reporting conventions, so the scenarios
presented in \cref{Chapter8} use \DIFdelbegin \DIFdel{system-level }\DIFdelend \DIFaddbegin \DIFadd{system level }\DIFaddend energy use estimates.
Overall, the impact estimating energy use at the \DIFdelbegin \DIFdel{system-level
including system-level }\DIFdelend \DIFaddbegin \DIFadd{system level
including system level }\DIFaddend on relative energy use is minor, as it scales
...
Therefore the \DIFdelbegin \DIFdel{high-level }\DIFdelend \DIFaddbegin \DIFadd{high level }\DIFaddend results are based on the simpler
\DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend methods. Results that emerge from spatial microsimulation
(and which would be inaccessible using \DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend methods alone) are presented
\DIFdelbegin \DIFdel{later-on}\DIFdelend \DIFaddbegin \DIFadd{later on}\DIFaddend , for a smaller case study region. South Yorkshire is used here
...
%DIF <  are used. This section therefore analyses the aggregate-level transport to work
%DIF >  are used. This section therefore analyses the aggregate level transport to work
...
route distance inferred from the \DIFdelbegin \DIFdel{the }\DIFdelend mode-distance combination and E an
estimate of the
...
\begin{itemize}
 ...
 ends of the 5-10 and \DIFdelbegin \DIFdel{and }\DIFdelend 10-20 km bins.
...
\end{itemize}
It would be desirable to corroborate these findings with other \DIFdelbegin \DIFdel{individual-level
}\DIFdelend \DIFaddbegin \DIFadd{individual level
}\DIFaddend data on travel to work. For the purposes of assessing the relative energy
...
\DIFdelbegin %DIFDELCMD < \begin{table}[htbp]
%DIFDELCMD < \caption[Average distance travelled by mode and distance band]
%DIFDELCMD < {Average distance travelled by mode and distance band (km),
%DIFDELCMD < from USd data.}\label{tdboxes}
%DIFDELCMD < \begin{center}
%DIFDELCMD < \begin{tabular}{lrrrrrrrr}
%DIFDELCMD < \toprule
%DIFDELCMD < Upper limit & 2.0 & 5.0 & 10.0 & 20.0 & 30.0 & 40.0 & 60.0 & 250.0 \\ \midrule
%DIFDELCMD < card & 1.6 & 3.9 & 7.9 & 15.0 & 26.0 & 35.8 & 50.3 & 102.6 \\
%DIFDELCMD < carp & 1.5 & 3.9 & 7.9 & 15.2 & 26.5 & 36.4 & 48.0 & 95.0 \\
%DIFDELCMD < moto & 1.4 & 4.1 & 7.0 & 15.2 & 23.5 & 36.0 & \multicolumn{1}{l}{NA} & \multicolumn{1}{l}{NA} \\
%DIFDELCMD < bus & 1.8 & 3.8 & 7.7 & 13.9 & 27.7 & 40.0 & 56.0 & 110.5 \\
%DIFDELCMD < train & 1.5 & 4.2 & 8.1 & 15.1 & 26.4 & 37.6 & 53.2 & 98.8 \\
%DIFDELCMD < metro & 1.7 & 4.0 & 8.1 & 14.7 & 25.8 & \multicolumn{1}{l}{NA} & \multicolumn{1}{l}{NA} & 65.0 \\
%DIFDELCMD < cycle & 1.5 & 3.9 & 7.5 & 11.5 & \multicolumn{1}{l}{NA} & \multicolumn{1}{l}{NA} & \multicolumn{1}{l}{NA} & \multicolumn{1}{l}{NA} \\
%DIFDELCMD < walk & 1.2 & 3.5 & 8.0 & 13.7 & 25.0 & \multicolumn{1}{l}{NA} & \multicolumn{1}{l}{NA} & \multicolumn{1}{l}{NA} \\
%DIFDELCMD < other & 1.0 & 4.3 & 7.6 & 13.5 & 27.8 & 37.5 & 42.0 & 130.0 \\
%DIFDELCMD < taxi & 1.7 & 3.0 & 9.0 & 12.0 & \multicolumn{1}{l}{NA} & \multicolumn{1}{l}{NA} & \multicolumn{1}{l}{NA} & \multicolumn{1}{l}{NA} \\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \end{center}
%DIFDELCMD < \end{table}
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[htbp]
%DIFDELCMD < \begin{center}
%DIFDELCMD <     \includegraphics[width=9cm]{dboxes}\end{center}
%DIFDELCMD <   \caption[Distance bands and average distance travelled for motorised modes]
%DIFDELCMD <   {Distance bands and average distance travelled for motorised modes, expressed
%DIFDELCMD <   as the relationship between lower bound and average distance (top)
%DIFDELCMD <   and that between lower bound and the ratio of upper bound to average distance
%DIFDELCMD <   (below), from Understanding Society data.} %%% could update with NTS!!! should have used centre...
%DIFDELCMD <   \label{fdboxes}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}[htbp]
\begin{center}
...
distance categories and \DIFdelbegin \DIFdel{tail-off }\DIFdelend \DIFaddbegin \DIFadd{tail off }\DIFaddend to essentially zero after the 20 km mark.
...
costs of areas within a $\sim$100 km radius \DIFdelbegin \DIFdel{surround }\DIFdelend \DIFaddbegin \DIFadd{surrounding }\DIFaddend the centre: commuters in Bedford, Essex and
...
It is interesting to \DIFdelbegin \DIFdel{not }\DIFdelend \DIFaddbegin \DIFadd{note }\DIFaddend where these extreme values are found:
...
found in \DIFdelbegin \DIFdel{in }\DIFdelend Park Farm North,
...
suggests that \DIFdelbegin \DIFdel{regional-level }\DIFdelend \DIFaddbegin \DIFadd{regional level }\DIFaddend assessments, such as those
...
within zones than between them. One pattern that stands out from the \DIFdelbegin \DIFdel{ward-level
}\DIFdelend \DIFaddbegin \DIFadd{ward level
}\DIFaddend analysis is the tendency of settlements to be directly surrounded by green areas
...
provided only at \DIFdelbegin \DIFdel{course }\DIFdelend \DIFaddbegin \DIFadd{coarse }\DIFaddend levels of aggregation, so using the spatial
...
the national level, something \DIFdelbegin \DIFdel{for }\DIFdelend which the spatial microsimulation methods
...
dataset is available from {\color{blue} \DIFdelbegin %DIFDELCMD < \href{https://www.gov.uk/government/statistical-data-sets/total-final-energy-consumption-at-regional-and-local-authority-level-2005-to-2010}{https://www.gov.uk/government/statistical-data-sets/}%%%
\DIFdelend \DIFaddbegin \href{https://www.gov.uk/government/statistical-data-sets/total-final-energy-consumption-at-regional-and-local-authority level-2005-to-2010}{https://www.gov.uk/government/statistical-data-sets/}\DIFaddend }
...
Because the \DIFdelbegin \DIFdel{national-level }\DIFdelend \DIFaddbegin \DIFadd{national level }\DIFaddend commuting dataset I was using operated at the
...
The only two Local \DIFdelbegin \DIFdel{authorities }\DIFdelend \DIFaddbegin \DIFadd{Authorities }\DIFaddend in which commuting was found to account
...
MacKay's figures and theirs.) Even without including the \DIFdelbegin \DIFdel{system-level
}\DIFdelend \DIFaddbegin \DIFadd{system level
}\DIFaddend energy costs of commuting described in \cref{Chapter5}, this is a large
...
\section{Local and \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend variability} \label{sindvar}

As with any research in which geographical zones are the unit of analysis,
the maps of energy use presented above mask \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend variability within
...
When distance band and mode of travel are \DIFdelbegin \DIFdel{know }\DIFdelend \DIFaddbegin \DIFadd{known }\DIFaddend but no cross-tabulations
...
A final potential issue with the \DIFdelbegin \DIFdel{ward-level }\DIFdelend \DIFaddbegin \DIFadd{ward level }\DIFaddend analysis of the entire nation, as presented
...
}\DIFdelend the `South Yorkshire conurbation' \citep{barker1978perthes} --- it has
...
with \DIFdelbegin \DIFdel{with }\DIFdelend wealthy (to the west of Sheffield city centre, bordering
...
\DIFdelbegin %DIFDELCMD < \begin{table}[htbp]
%DIFDELCMD < \caption[Sample of individual-level spatial microsimulation output]
%DIFDELCMD < {Sample of individual-level microsimulation output. The number of cars
%DIFDELCMD < in the individuals' household and the engine size of their primary car
%DIFDELCMD < are extracted using the merge() ~function applied to the ID codes,
%DIFDELCMD < that are also present in \cref{tintallh}}
%DIFDELCMD < \begin{tabular}{rrrrrlr}
%DIFDELCMD < \toprule
%DIFDELCMD < \multicolumn{1}{l}{} & \multicolumn{1}{l}{a\_hidp} & \multicolumn{1}{l}{a\_pno} & \multicolumn{1}{l}{pidp} & \multicolumn{1}{l}{N.~cars} & Engine size & \multicolumn{1}{l}{Et} \\ \midrule
%DIFDELCMD < 18 & 68041483 & 2 & 68041491 & 2 Sheffield& medium engine - 1.4 - 1.9999 & 312.3 \\
%DIFDELCMD < 18 & 68041483 & 2 & 68041491 & 2 & small engine - 1.0 - 1.3999 & 268.3 \\
%DIFDELCMD < 200 & 68303283 & 1 & 68303287 & 1 & inapplicable & 743.8 \\
%DIFDELCMD < 200 & 68303283 & 1 & 68303287 & 1 & small engine - 1.0 - 1.3999 & 471.7 \\
%DIFDELCMD < 219 & 68323003 & 1 & 68323007 & 1 & inapplicable & 423.0 \\
%DIFDELCMD < 219 & 68323003 & 1 & 68323007 & 1 & medium engine - 1.4 - 1.9999 & 312.3 \\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \label{tintcar}
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{table}[htbp]
...
The \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend results are well-illustrated by plotting the proportion
...
the lowest classes \DIFdelbegin \DIFdel{live }\DIFdelend \DIFaddbegin \DIFadd{tend to work }\DIFaddend closer to home, on average, than the averages for
...
demonstrate that \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend variability in commuter energy use
...
\begin{itemize}
 ...
 \DIFdelbegin \DIFdel{set-out }\DIFdelend \DIFaddbegin \DIFadd{set out }\DIFaddend to test was that the Netherlands would have lower energy costs, primarily
...
Another issue was finding \DIFdelbegin \DIFdel{Geographical }\DIFdelend \DIFaddbegin \DIFadd{geographical }\DIFaddend data for Dutch provinces and their
...
which are \DIFdelbegin \DIFdel{the }\DIFdelend also on opposite ends of the population density scale.
...
A problem with the preceding \DIFdelbegin \DIFdel{national-level }\DIFdelend \DIFaddbegin \DIFadd{national level }\DIFaddend comparison is that the
...
\DIFdelbegin \DIFdel{also}\DIFdelend \DIFaddbegin \DIFadd{as well}\DIFaddend . %!!! add a link here
That energy use per commute is greater in the Netherlands
\DIFdelbegin \DIFdel{is
higher }\DIFdelend than in England is an interesting result in itself and merits
...
were used to \DIFdelbegin \DIFdel{fill-in }\DIFdelend \DIFaddbegin \DIFadd{fill in }\DIFaddend the gap. Finally, the figures for the proportion of people
...
From inspection of the district and \DIFdelbegin \DIFdel{ward-level }\DIFdelend \DIFaddbegin \DIFadd{ward level }\DIFaddend maps, it is clear that dense
...
by mode, cross tabulated by household income \DIFaddbegin \DIFadd{(}\DIFaddend \cref{fig:income-dis}\DIFaddbegin \DIFadd{)}\DIFaddend . Transport
...
models in the \DIFdelbegin \DIFdel{1960's}\DIFdelend \DIFaddbegin \DIFadd{1960s}\DIFaddend : ``disillusionment with technology began to grow as
...
variability in distance \DIFaddbegin \DIFadd{(}\DIFaddend \cref{income-heatmap}\DIFaddbegin \DIFadd{)}\DIFaddend .
...
spatial microsimulation really come into their own, as aggregate data \DIFdelbegin \DIFdel{tells
}\DIFdelend \DIFaddbegin \DIFadd{tell
}\DIFaddend us little about the socio-economic attributes of the individuals
...
This trend --- towards \DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend social and spatial analysis
...
\item the possibility of modelling the impacts of \DIFdelbegin \DIFdel{small-scale }\DIFdelend \DIFaddbegin \DIFadd{small scale }\DIFaddend interventions
...
The shift towards \DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend analysis also has some potential
disadvantages. These limitations,
and strategies to overcome them, can be summarised as follows:
\begin{itemize}
 \item The \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend results are simulated, and are unlikely to be
...
more accurate origin-destination pairs for individuals. \DIFdelbegin \DIFdel{Lower-level }\DIFdelend \DIFaddbegin \DIFadd{Lower level }\DIFaddend commuter
...
A more fundamental barrier preventing the use of \DIFdelbegin \DIFdel{micro-level
}\DIFdelend \DIFaddbegin \DIFadd{micro level
}\DIFaddend methods in many contexts is that accurate, geocoded
...
%DIF <  policy makers who lack access to official, geocoded microdata (individual-level
%DIF >  policy makers who lack access to official, geocoded microdata (individual level
...
%DIF <  % This raises the following question: how can micro-level analyses of
%DIF >  % This raises the following question: how can micro level analyses of
...
% The aim
The more practical aim of this section is to bring \DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend analysis
...
%DIF <  a non-geograhical example of city-level aggregation,
%DIF >  a non-geograhical example of city level aggregation,
...
%DIF <  (geographic) \emph{and} micro (individual-level) factors would be
%DIF >  (geographic) \emph{and} micro (individual level) factors would be
...
% % \citep{Vega2011}
%DIF <  Increasingly, newly available micro-level datasets are being incorporated
%DIF >  Increasingly, newly available micro level datasets are being incorporated
...
%DIF <  % This micro-level section needs a bit of work mate (5th Sept)
%DIF >  % This micro level section needs a bit of work mate (5th Sept)
...
%DIF <  % in the developing world.} yet lower-level factors such as international
%DIF >  % in the developing world.} yet lower level factors such as international
...
% 
%DIF <  A couple of national-level studies serve here to illustrate the utility of
%DIF >  A couple of national level studies serve here to illustrate the utility of
...
%DIF <  individual-level geolocated database of all 2 million workers to calculate
%DIF >  individual level geolocated database of all 2 million workers to calculate
...
%DIF <  Another recent application of individual-level geolocated census data to
%DIF >  Another recent application of individual level geolocated census data to
...
% \ref{Methods}.
% 
%DIF <  With the increasing availability of individual-level transport
%DIF >  With the increasing availability of individual level transport
...
%DIF <  How can methods of individual-level transport analysis be
%DIF >  How can methods of individual level transport analysis be
% presented and disseminated such that they are used by
%DIF <  others?\footnote{The ArcGIS-based methods of individual-level analysis and
%DIF >  others?\footnote{The ArcGIS-based methods of individual level analysis and
...
%DIF <  Spatial microsimulation allocates individual-level data to areas (see
%DIF >  Spatial microsimulation allocates individual level data to areas (see
...
\DIFdelbegin %DIFDELCMD < \begin{table}[htbp]
%DIFDELCMD < \caption[Aggregate-level inputs into the spatial microsimulation model]
%DIFDELCMD < {The four constraint variables and their associated categories used as
%DIFDELCMD < the aggregate-level inputs into the spatial microsimulation model.
%DIFDELCMD < The category notation for numeric variables follows
%DIFDELCMD < the International Organization for Standardization
%DIFDELCMD < \href{http://www.iso.org/iso/catalogue_detail?csnumber=31887}
%DIFDELCMD < {(ISO) 80000-2:2009}:
%DIFDELCMD < Square brackets indicate that the endpoint is not included in the set,
%DIFDELCMD < curved brackets indicate that the endpoint is included.}
%DIFDELCMD < 

%DIFDELCMD < \begin{center}
%DIFDELCMD < \begin{tabular}{lrp{3cm}p{8cm}}\toprule
%DIFDELCMD < Variable & \multicolumn{1}{l}{N. } & Categories/bin breaks & Comments \\ \midrule
%DIFDELCMD < Age/sex & 12 & (16,20] (20,25] (25,35] (35,55] (55,100] & Female and male categories, in employment (excludes full-time students) \\
%DIFDELCMD < Mode & 11 & mfh       metro     train     bus       moto      car.d     car.p
%DIFDELCMD < taxi      cycle     walk      other & “Main mode of travel to work” (no data on variability of mode choice) \\
%DIFDELCMD < Distance & 8 & (0,2] (2,5] (5,10] (10,20] (20,30] (30,40] (40,60] (60,250] &
%DIFDELCMD < Euclidean distance between respondents' home postcode and their main place of work (does not capture multiple work destinations) \\
%DIFDELCMD < NS-SEC & 9 & NS-SEC 1.1, 1.2 2, 3, 4, 5, 6, 7 and other & Classes range
%DIFDELCMD < from higher managerial (NS-SEC  1.1) to routine occupations (NS-SEC 7) --- see
%DIFDELCMD < \citep{chandola2000new} and on the ONS website
%DIFDELCMD < (\href{http://www.ons.gov.uk/ons/guide-method/classifications/current-standard-classifications/soc2010/soc2010-volume-3-ns-sec--rebased-on-soc2010--user-manual/index.html}{www.ons.gov.uk}) \\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}\end{center}
%DIFDELCMD < \label{t:constraints}
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{table}[htbp]
...
\mbox{%DIFAUXCMD
\citep{chandola2000new}
}%DIFAUXCMD
and on the ONS website
...
dataset for analysis using various techniques (e.g.~\DIFdelbegin \DIFdel{individual-level}\DIFdelend \DIFaddbegin \DIFadd{individual level}\DIFaddend ,
...
% correlates of energy intensive commuter patterns.
The spatial microsimulation model results in a large dataset containing
hundreds of individuals for each zone under investigation. For \DIFdelbegin \DIFdel{micro-level
}\DIFdelend \DIFaddbegin \DIFadd{micro level
}\DIFaddend spatial analysis, origin-destination pairs are needed: simulated
places of home and work need to be geotagged. The simplest solution
to this problem is to allocate all individuals in each zone home coordinates
...
centre, a more realistic approach is to acknowledge that a variety of
employment centres exist, and that the relative importance of each varies from
place to place. This is illustrated in Fig.~\ref{fig:sflow}, a \DIFdelbegin \DIFdel{ward-level
}\DIFdelend \DIFaddbegin \DIFadd{ward level
}\DIFaddend flow diagram of
the work locations of commuters based on the outskirts of Sheffield. Although
Barnsley is the closest city centre to Stocksbridge (see
Fig.~\ref{fig:dis-msoa}), this analysis makes it clear that Sheffield is the
primary non-home workplace.
...
hence our limitation to \DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend analysis of this dataset here.
}
...
are by nature detailed and provided at the \DIFdelbegin \DIFdel{individual-level}\DIFdelend \DIFaddbegin \DIFadd{individual level}\DIFaddend , validation
...
The first of \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend is relatively
...
target variables that are the \DIFdelbegin \DIFdel{the }\DIFdelend most useful result,
...
levels, let alone the \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend geocoded data required to validate the
\DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend results of the spatial microsimulation model. Access to such
...
%DIF <  \item Compare aggregate-level results of the model with census variables that
%DIF >  \item Compare aggregate level results of the model with census variables that
...
than \DIFdelbegin \DIFdel{than }\DIFdelend the average. Car drivers and home workers tend to be slightly older.
\item Car ownership, which is seldom \DIFdelbegin \DIFdel{factored-into }\DIFdelend \DIFaddbegin \DIFadd{factored into }\DIFaddend transport policy assessments,
\citep{Kay2011}
...
are passengers in other peoples' cars). The utility of the \DIFdelbegin \DIFdel{individual-level
}\DIFdelend \DIFaddbegin \DIFadd{individual level
}\DIFaddend results is illustrated at this aggregate level by observing differences in
...
\DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend variables such as income.
...
Statistical bins are defined as proportions of the \DIFdelbegin \DIFdel{the }\DIFdelend median income, with
...
indicates that \DIFdelbegin \DIFdel{the }\DIFdelend Sheffield028 (an MSOA zone) is more unequal in terms of
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[h*]
%DIFDELCMD <  \centering
%DIFDELCMD < \includegraphics[width=6.5cm]{cat-vars}
%DIFDELCMD < \includegraphics[width=6.5cm]{Lorenz3}
%DIFDELCMD <  \caption[a) Income and household traits; b) Lorenz curves of commute distances]
%DIFDELCMD <  {\textbf{a)} Variability of vehicles (proportion of primary cars in household
%DIFDELCMD < whose engine size is 2.0 litres or more), internet use (proportion of
%DIFDELCMD < commuters who use the internet daily or weekly) and desire to move home
%DIFDELCMD < depending on equivalised income. These categorical target variables are
%DIFDELCMD < described in Table \ref{t:indata}. \textbf{b)} Lorenz curves illustrating the
%DIFDELCMD < individual-level variability in
%DIFDELCMD < commuter distances for 3 zones. The Gini indices associated with these curves
%DIFDELCMD < are 0.278, 0.294 and 0.305 for Stocksbridge, South Yorkshire and Sheffield028
%DIFDELCMD < respectively.}
%DIFDELCMD <  \label{fig:cat-vars}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}[h*]
 \centering
...
illustrates the importance of considering not only \DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend impacts, but
also taking into account the local and \DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend distributional effects of
intervention.
...
These depend on geographical \emph{and} \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend factors,
and the importance of the latter especially is often overlooked in transport policy
 (e.g.~\citealp{Tribby2012}).
The \DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend methods presented in this chapter therefore have great potential,
...
boxes'', and not \DIFdelbegin \DIFdel{set-up }\DIFdelend \DIFaddbegin \DIFadd{set up }\DIFaddend for the particular (and often unique) problem that they
...
`Going Dutch' is implemented both at aggregate and \DIFdelbegin \DIFdel{individual-levels}\DIFdelend \DIFaddbegin \DIFadd{individual levels}\DIFaddend ,
to highlight the advantages of each approach: the \DIFdelbegin \DIFdel{aggregate-level
}\DIFdelend \DIFaddbegin \DIFadd{aggregate level
}\DIFaddend scenario allows energy savings to be calculated nationwide, while the
\DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend implementation allows more sophisticated determination
...
assumed in the \DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend version. The code used to generate and analyse
...
\subsection{An \DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend model of modal shift}
...
lots of start stop) and that was ok most of the time. Done a \DIFaddbegin [\DIFadd{sic}] \DIFaddend asymmetric 17.5
miles there 13 miles back and that started to feel bit of a drag in terms of
time and \DIFdelbegin \DIFdel{and }\DIFdelend effort. I think for me 15 miles each way would be my limit for 5
days a week.
Although at the moment I'm very lazy and drive 8 miles to work 3 days a week and
ride \DIFdelbegin \DIFdel{ride }\DIFdelend 2days a week!''
}
...
the proportion of car trips \DIFdelbegin \DIFdel{that shift to car }\DIFdelend \DIFaddbegin \DIFadd{replaced by bicycle }\DIFaddend trips, $EI$ is the energy
...
proportion of all trips is not as high in this scenario \DIFdelbegin \DIFdel{is not as high as }\DIFdelend \DIFaddbegin \DIFadd{as }\DIFaddend might
...
willing to \DIFdelbegin \DIFdel{take-up }\DIFdelend \DIFaddbegin \DIFadd{take up }\DIFaddend cycling), infrastructure or terrain. A number of refinements
...
cycling could be modelled at the \DIFdelbegin \DIFdel{individual-level}\DIFdelend \DIFaddbegin \DIFadd{individual level}\DIFaddend , as a function of
...
of cycling uptake is driven largely \DIFdelbegin \DIFdel{be }\DIFdelend \DIFaddbegin \DIFadd{by }\DIFaddend the
young.\footnote{Sex
...
An additional problem with the \DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend model is that it depends
...
in \cref{Chapter4} are used to create an \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend model of modal shift.
To take the age-dependence into account, the \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend implementation
...
simple linear model was used to normalize the \DIFdelbegin \DIFdel{the }\DIFdelend previous probability
estimates by age.
...
As with the \DIFdelbegin \DIFdel{national-level }\DIFdelend \DIFaddbegin \DIFadd{national level }\DIFaddend figures, the savings vary geographically: the lowest
...
\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD <  \centering{\includegraphics[width = 12cm]{cysave-ind}}
%DIFDELCMD <  \caption[Energy savings from car-bike modal shift in South Yorkshire]
%DIFDELCMD <  {Energy savings from car-bike modal shift in South Yorkshire, from the
%DIFDELCMD <  individual-level implementation of the `go Dutch' scenario.} \label{fbikeage}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}
...
Across the region as a whole, the difference between the \DIFdelbegin \DIFdel{individual-level
}\DIFdelend \DIFaddbegin \DIFadd{individual level
}\DIFaddend implementation (with age dependency) and the simplistic implementation
...
range of distances within distance bands: in the \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend implementation
a 9 km trip is less likely to switch to bicycle than a 6 km trip whereas
in the \DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend model the probability is the same. The spatial distribution
of the differences between the estimated energy savings in the \DIFdelbegin \DIFdel{individual-level
and aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{individual level
and aggregate level }\DIFaddend implementations are shown in \cref{fcydif}. Note that the
\DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend savings were substantially lower in Stocksbridge (Northwest
Sheffield).
...
commuter energy costs are high. (In the \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend implementation
...
\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD <  \centering{\includegraphics[width = 12cm]{cysave-diff}}
%DIFDELCMD <  \caption[Differences between individual and aggregate-level implementations]
%DIFDELCMD <  {Differences between individual and aggregate-level implementations of the
%DIFDELCMD <  `go Dutch' scenario across the MSOA zones of South Yorkshire.} \label{fcydif}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}
...
with origin-destination points and accurate zonal and \DIFdelbegin \DIFdel{individual-level
}\DIFdelend \DIFaddbegin \DIFadd{individual level
}\DIFaddend variables. Developing such a model would be an application of the spatial
...
(Dore in the West of Sheffield, for example is notoriously \DIFdelbegin \DIFdel{well-off}\DIFdelend \DIFaddbegin \DIFadd{well off}\DIFaddend , and has large savings
...
and represented as part \DIFdelbegin \DIFdel{part }\DIFdelend of an integrated land-use transport model.
...
neighbourhood without commuting'' \citep[p.~591]{North2010585}.
 ...
 }\DIFdelend \item Second locally useful professions will become common,
...
highlights how commuting is \DIFdelbegin \DIFdel{bound-up }\DIFdelend \DIFaddbegin \DIFadd{bound up }\DIFaddend in the wider economy and illustrates
...
%DIF <  Therefore, while the scenarios set-out in this case study of new routes along
%DIF >  Therefore, while the scenarios set out in this case study of new routes along
...
%DIF <  dataset (USd), an individual-level nationwide survey of socio-economic variables
%DIF >  dataset (USd), an individual level nationwide survey of socio-economic variables
...
%DIF <  %zonal and individual-level variables (see \citealp{Ballas2005c}).
%DIF >  %zonal and individual level variables (see \citealp{Ballas2005c}).
% % The model
% % selects individuals based on 4 constraints (or `linking variables') which are
%DIF <  % shared between the individual-level Understanding Society dataset (USd) and
%DIF >  % shared between the individual level Understanding Society dataset (USd) and
...
focus on the \DIFdelbegin \DIFdel{the }\DIFdelend direct monetary costs:
...
oil vulnerability and, after exploring \DIFdelbegin \DIFdel{aggregate-level
}\DIFdelend \DIFaddbegin \DIFadd{aggregate level
}\DIFaddend findings based on census data, investigate the results of each formulae in turn.
...
%DIF <  mask variation at the individual level \citep{Openshaw1983}. Individual-level
%DIF >  mask variation at the individual level \citep{Openshaw1983}. Individual level
% data illustrate the inequalities that exist between people, and the links
%DIF <  between commuting and other individual-level variables such as income,
%DIF >  between commuting and other individual level variables such as income,
...
%DIF <  survey (individual-level) and census (regional) data \citep{Ballas2005b}. The
%DIF >  survey (individual level) and census (regional) data \citep{Ballas2005b}. The
...
(r) }\DIFaddend ranged from -0.59 to -0.22
...
for negative correlations: deprivation is primarily \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend urban phenomenon in
...
A key advantage of spatial microsimulation, however, is that \DIFdelbegin \DIFdel{individual-level
}\DIFdelend \DIFaddbegin \DIFadd{individual level
}\DIFaddend characteristics can be modelled.
...
($V_{cfp}$)
score due to high income. Their \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend energy vulnerability ($V_{ei}$)
...
insulated house) who travels a few kilometres to work may have \DIFdelbegin \DIFdel{have }\DIFdelend a low
...
\DIFdelbegin %DIFDELCMD < \begin{table}[htbp]
%DIFDELCMD < \caption[Individual-level characteristics
%DIFDELCMD < of `oil vulnerable' commuters]{Individual-level characteristics
%DIFDELCMD < of `oil vulnerable' commuters in living in the three zones of
%DIFDELCMD < York depicted in Fig.~\ref{york}, estimated by the spatial microsimulation
%DIFDELCMD < model.}
%DIFDELCMD < \begin{center}
%DIFDELCMD < \begin{tabular}{|c|l|r|r|r|}
%DIFDELCMD < \hline
%DIFDELCMD < Subset & Statistic & \multicolumn{1}{l|}{1: Central} & \multicolumn{1}{l|}{2:
%DIFDELCMD < Suburb} & \multicolumn{1}{l|}{3: Outskirts} \\ \hline
%DIFDELCMD < \multicolumn{ 1}{|c|}{$V{cfp}$} & N & 241 & 51 & 151 \\ \cline{ 2- 5}
%DIFDELCMD < \multicolumn{ 1}{|c|}{$\geq 10\%$} & Average age & 39 & 41 & 44 \\ \cline{ 2- 5}
%DIFDELCMD < \multicolumn{ 1}{|c|}{} & Average income & 19100 & 15800 & 14300 \\ \cline{ 2-
%DIFDELCMD < 5}
%DIFDELCMD < \multicolumn{ 1}{|c|}{} & Income SD & 9400 & 10100 & 8400 \\ \cline{ 2- 5}
%DIFDELCMD < \multicolumn{ 1}{|c|}{} & N. children & 0.51 & 0.88 & 0.67 \\ \cline{ 2- 5}
%DIFDELCMD < \multicolumn{ 1}{|c|}{} & \% drive to work & 45 & 53 & 56 \\ \hline
%DIFDELCMD < \multicolumn{ 1}{|c|}{$V{e}$} & N & 1168 & 990 & 2466 \\ \cline{ 2- 5}
%DIFDELCMD < \multicolumn{ 1}{|c|}{$\geq 10\%$} & Average age & 35 & 31 & 41 \\ \cline{ 2- 5}
%DIFDELCMD < \multicolumn{ 1}{|c|}{} & Average income & 18000 & 16600 & 19200 \\ \cline{ 2-
%DIFDELCMD < 5}
%DIFDELCMD < \multicolumn{ 1}{|c|}{} & Income SD & 11900 & 8000 & 10300 \\ \cline{ 2- 5}
%DIFDELCMD < \multicolumn{ 1}{|c|}{} & N. children & 0.67 & 0.64 & 0.67 \\ \cline{ 2- 5}
%DIFDELCMD < \multicolumn{ 1}{|c|}{} & \% drive to work & 43 & 40 & 61 \\ \hline
%DIFDELCMD < \multicolumn{ 1}{|c|}{All } & N & 4085 & 3091 & 4424 \\ \cline{ 2- 5}
%DIFDELCMD < \multicolumn{ 1}{|c|}{commuters} & Average age & 36 & 41 & 42 \\ \cline{ 2- 5}
%DIFDELCMD < \multicolumn{ 1}{|c|}{} & Average income & 19500 & 17900 & 19686 \\ \cline{ 2-
%DIFDELCMD < 5}
%DIFDELCMD < \multicolumn{ 1}{|c|}{} & Income SD & 12600 & 10800 & 12000 \\ \cline{ 2- 5}
%DIFDELCMD < \multicolumn{ 1}{|c|}{} & N. children & 0.56 & 0.7 & 0.67 \\ \cline{ 2- 5}
%DIFDELCMD < \multicolumn{ 1}{|c|}{} & \% drive to work & 25 & 49 & 61 \\ \hline
%DIFDELCMD < \end{tabular}\end{center}
%DIFDELCMD < \label{t:ind2}
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{table}[htbp]
...
By providing estimates for a range of \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend variables, spatial
...
co-benefit of \DIFdelbegin \DIFdel{freeing-up }\DIFdelend \DIFaddbegin \DIFadd{freeing up }\DIFaddend oil for shorter commutes or public transport). The
...
have been evaluated in energy terms. As \DIFdelbegin \DIFdel{set-out }\DIFdelend \DIFaddbegin \DIFadd{set out }\DIFaddend in \cref{Chapter1}, the
...
whereas those who \DIFdelbegin \DIFdel{take-up }\DIFdelend \DIFaddbegin \DIFadd{take up }\DIFaddend telecommuting use on average more than 3 times more
...
and \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend methodologies (\cref{Chapter3}), it was decided that
...
with \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend data from the national Understanding Society dataset,
...
analysis, visualisation, testing and modelling \DIFdelbegin \DIFdel{also}\DIFdelend \DIFaddbegin \DIFadd{as well}\DIFaddend .
% To produce estimates of energy use at the
%DIF <  individual-level and at small geographical scales a spatial microsimulation
%DIF >  individual level and at small geographical scales a spatial microsimulation
...
been learned during the \DIFdelbegin \DIFdel{course of the }\DIFdelend research project:
...
technique. \DIFdelbegin \DIFdel{Individual-level }\DIFdelend \DIFaddbegin \DIFadd{Individual level }\DIFaddend analysis is becoming the norm in
...
\DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend studies, such as the targeting of specific groups in what-if
...
\DIFdelbegin \DIFdel{national-level }\DIFdelend \DIFaddbegin \DIFadd{national level }\DIFaddend results presented in parts \cref{Chapter6} and \cref{Chapter8}),
...
On the contrary, the scenario of modal shift shows that \DIFdelbegin \DIFdel{aggregate-level
}\DIFdelend \DIFaddbegin \DIFadd{aggregate level
}\DIFaddend analysis can be useful for a rapid assessment of the basic determinants of
...
using spatial microsimulation). The progression from aggregate to \DIFdelbegin \DIFdel{micro-level
}\DIFdelend \DIFaddbegin \DIFadd{micro level
}\DIFaddend undertaken in this scenario illustrates the benefits of using a \DIFdelbegin \DIFdel{micro-level
}\DIFdelend \DIFaddbegin \DIFadd{micro level
}\DIFaddend approach in tandem with preliminary \DIFdelbegin \DIFdel{aggregate-level }\DIFdelend \DIFaddbegin \DIFadd{aggregate level }\DIFaddend analysis.
The \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend implementation of the scenario, based on spatial microsimulation,
...
In the process of moving from an aggregate to a \DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend model of
...
by individuals (\cref{s:commuting}), so a \DIFdelbegin \DIFdel{micro-level }\DIFdelend \DIFaddbegin \DIFadd{micro level }\DIFaddend approach makes sense in theory.
...
between geographical and \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend factors to be included.
...
between academics and practitioners, by making individual and \DIFdelbegin \DIFdel{local-level
}\DIFdelend \DIFaddbegin \DIFadd{local level
}\DIFaddend analysis of personal travel patterns accessible.
...
it is \DIFdelbegin \DIFdel{it is }\DIFdelend complex,\footnote{Spatial
...
\DIFdelbegin \DIFdel{Individual-level }\DIFdelend \DIFaddbegin \DIFadd{Individual level }\DIFaddend variability was also explored in the
...
it is \DIFdelbegin \DIFdel{recommend}\DIFdelend \DIFaddbegin \DIFadd{recommended}\DIFaddend , if an overriding agenda or
...
more complex at the local level. In terms of \DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend variability, it
...
of \DIFdelbegin \DIFdel{scenarios of }\DIFdelend change, respectively. The following bullet points summarise
...
Aim 1.1 was also to investigate \DIFdelbegin \DIFdel{household-level }\DIFdelend \DIFaddbegin \DIFadd{household level }\DIFaddend variability. This has not been
...
\DIFdelbegin \DIFdel{individual-level }\DIFdelend \DIFaddbegin \DIFadd{individual level }\DIFaddend methodologies compared with more common \DIFdelbegin \DIFdel{aggregate-level
}\DIFdelend \DIFaddbegin \DIFadd{aggregate level
}\DIFaddend approaches. The ability to target specific groups in scenarios of change, to
...
aims, although further work is needed to include \DIFdelbegin \DIFdel{household-level }\DIFdelend \DIFaddbegin \DIFadd{household level }\DIFaddend impacts and
...
emphasis on bicycles, for example, is \DIFdelbegin \DIFdel{at-odds }\DIFdelend \DIFaddbegin \DIFadd{at odds }\DIFaddend with its relatively
