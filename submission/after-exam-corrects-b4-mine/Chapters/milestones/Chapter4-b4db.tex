...
% This PhD is a data-driven exploration of the energy costs of commuting.
...
The ideal dataset would contain grid references of both the origin and
...
the 3$^{rd}$ quarter of 2012, there were 29.86
employed
...
infrastructure that is currently beyond government means. Large data management
...
The largest microsimulation run performed for this PhD was of $\sim$2 million
...
In this PhD we take the opposite view: that the inherent data limitations mean
that the datasets that \emph{are} available are
absolutely critical. Systematically collected data has a much better chance of
meeting the PhD's aims, as set-out in \cref{s:aims} than purely qualitative
information. Without good statistics, we would have to resort personal
...
the datasets (\cref{senergyusedata} to \cref{sadditional}) and then
...
the majority of the PhD. In this chapter the emphasis reverses, and the methods
...
aims of this PhD. Yet effort has been made to explain them in general terms. An
...
cornerstones
of scientific advancement yet is often missing in spatial microsimulation and
related fields. Therefore, this is probably
the most important chapter in the Thesis from an academic perspective. It
allows others build on the analysis, by applying the methods to new datasets
...
The methods presented in this PhD operate the other
...
patterns can be modelled. The main datasets used in this PhD are:
\begin{itemize}
 \item Transport energy use data from a variety of sources % Yes
 \item The 2001 Census % Yes - not all working... - should add other censuses!
...
As we shall see, each has their advantages and disadvantages. The census
dataset is the most geographically comprehensive
...
national survey, so has a wide range of socio-economic and attitudinal variables
...
The first data source to be described is the energy
use of personal travel, as good energy data is vital to
the results of the PhD. Although energy use can be calculated based on
mode, distance and other variables, some official sources also provide energy
data directly. Energy use in transport is, in
...
between primary and final energy). Official data on the subject therefore also
...
The energy costs of personal travel and hence commuting are not recorded in
the same way as
...
\citep{Coley2002, Brand2006, Lovelace2011}.

As indicated in \cref{Chapter1}, %???,
the energy costs of commuting have not been previously analysed in detailed.
...
easily converted into Joules, the energy unit used in this PhD using the
...
benefits, the data is temporal: it would allow changes in the geographical
...
total. A simple way of doing this is to simply multiply the all fuel use values
...
\%). This data is from 2010 and can be found in Table NTS0402 from
...
methods of translating the total in into commuter energy use only could be used,
...
is the national census, which must be answered by every household. The data is
...
This data is provided for a 10\% sample before 2001, which is
problematic in small areas.\footnote{The data is provided down to enumeration
...
use of census data, therefore, was as a source of constraint variables
for the spatial microsimulation
models, at various levels of geographic aggregation. The disadvantage of census
data is that it only provides information about a small number of variables
...
by a range of administrative boundaries. Yorkshire and the Humber (YatH, left),
...
more difficult to analyse and visualise. 
...
variables' that are available through a single dataset: the
...
Crucially for this PhD, the USd also contains data on travel to work.
...
British longitudinal survey (Buck, personal
...
information about personal travel (but less more general data) we turned to the
...
The household-level data is most useful at providing insight into people's
...
presented in this PhD. They could, however, be useful for evaluating the
impact of
...
\begin{itemize}
 \item  Variable i182A --- Driving licence (yes, no or provisional): this is
useful in separating those who
...
mentioned so far deal with travel to work directly, however. Commuting data is
...
Travel to work data is usually a gross oversimplification of
...
\section{Additional input data} \label{sadditional}
The data presented so far, on energy use of personal travel and commuting
behaviour, is sufficient to calculate the energy costs of commuting at
individual and aggregate levels. The scope of this PhD extends beyond mere
description, however. Additional input data is required to explain \emph{why}
...
\subsection{Infrastructure and topography data}
...
pre-defined path. Thus, although trains (and to a limited extent buses, when
...
was found that OSM data is generally accurate, especially for large
...
were also noted, with deprived areas tending to be more poorly represented in
...
However, large errors were far rarer than small ones and overall the OSM
dataset was evaluated as being of `very good' quality; this is especially
impressive that official datasets are also susceptible to problems.
\citet{Haklay2010} highlighted the dynamic nature of OSM data, pointing
openstreetbugs as an example of how data validation can also take advantage of
the millions of remote sensing and data processing machines that are
increasingly connected to the internet and aware of collaborative online
efforts. If the same analysis were conducted now, it would likely be even more
favourable to OSM data.
...
The final point to consider is usability. While OSM data is available
worldwide, it is not the standard dataset in use by local planning departments,
which generally have institutional access to Ordnance Survey data. OSM data is
generally also more difficult for non-expert users to find and
download.\footnote{The
OSM transport data presented in \cref{fosm-trans}, for example, was not
...
(OSM data is well suited to use in geo-databases such as PostGIS.)} Therefore,
...
useful for low-level analysis of likely routes of non-motorised. However, the
former was found to be difficult to use\footnote{97 tiles were downloaded
for the wider Cambridge area to investigate this dataset. Loading the .gml file
in QGIS took over a minute and eventually crashed the computer.} and the latter
appears to be unavailable under an academic licence.
...
Topographic data for the UK is available from the the following sources,
ranging from the coarsest to the finest:
\begin{itemize}
\item The Advanced Spaceborne Thermal Emission and Reflection Radiometer
(ASTER) sensor mounted on the Space Shuttle has produced data that has been
...
{http://www.jspacesystems.or.jp}. Digital elevation data was successfully
...
ranging) data for the majority of the island of Great Britain. This data can be
...
commercially for £100 per square kilometre, or free for academic purposes. 
\end{itemize}
These datasets were not used in the PhD. Their inclusion could, however, provide
interesting avenues for further research (e.g.~as a
predictor of the rate of cycling and walking or as a local modifier of energy
economy estimates).
...
of the average distance to jobs. Distance to nearest employment centre is a
potentially useful proxy to measure this type of remoteness. This, and
related classification of areas, form the basis of this section. All have the
ultimate aim of helping to explain energy use in commuting different types of
area, later in the thesis. %Has this happened???!!!
...
The starting point for this analysis to consider the opposite of remoteness:
...
To tackle this issue the concept of `employment centre' was used
...
\verb nncross \ from the package spatstat was used to produce the correct
output. These results were converted back into QGIS files, and corroborated
manually. The resulting Euclidean distances are depicted in
...
always undertaken by individual actors making decisions over which they have
...
The technique used in this PhD is Iterative Proportional Fitting (IPF), which
...
\subsection{Why R?}
The majority of the quantitative analysis conducted for this PhD, and the
...
analysis. The second category summarises the use of R in this thesis perfectly:
...
scripts in R rather than a chaotic collection of unrelated functions: by its
...
are modular. This capability is not unique to R. However, the range statistical
functions that can be performed with a unified modelling framework is. The
...
The final reason for using R is its ability to interact with other programming
languages and environments. This work is in its infancy, yet there are already
promising projects that build on R's strengths and overcome some of its weaknesses.
These strengths-by-association, which tackle R's (standalone)
...
\begin{itemize}
 ...
 to overcome by querying databases (stored on the hard-disk) from within R, including
 RMySQL \citep{Chrisbare2011} and Rattle \citep{Williams2009}. \citet{Singleton2013-school}
 ...
 \item R is slow. At least it is at running \verb for \ loops and using R as if it were
 a general-purpose programming language (which it is not).
 However, there are times when the
 performance advantages of a compiled language such as C/C++ are needed (R is interpreted).
 ...
 ggplot2 has been used throughout this PhD for plotting with help from key
 reference \citep{wickham2011ggplot2, chang2012r}.
...
disaggregated conditional probabilities for the individual attributes of interest.
...
Code to read-in the individual-level data is presented in code sample \ref{cusd}.
...
same number of people. This is logical (e.g.~how can there be more people classified
...
by equivalised household income. However, the trip cost estimates were per year
and the income estimates are supplied per month in the USd. It took several
...
validation, is inherently limited by lack of real spatial microdata. However,
...
of comparing the model results against knowledge of  of how it \emph{should}
perform \emph{a-priori} (model checking). Second, we compare the
internally consistent model results with external empirical data (validation).
...
quantitative analysis. They ensure that the data is properly
...
we would know that the input data for commuters was faulty.
...
sophisticated plots (see \cref{fig:IPF-4c}, above).
...
\begin{figure}[h]
  \includegraphics[width=10cm, bb=0 0 1113 529]{errors3-1}
 % cc-trans.png: 1113x529 pixel, 72dpi, 39.26x18.66 cm, bb=0 0 1113 529
...
of the assumptions on which they are built, and the confidence we should have
in the results are important. This is especially true of models designed to inform
policies which have the potential to affect human lives. Yet
...
on which controlled experiments can be performed, t
he difficulty of model evaluation can
...
\begin{itemize}
 \item The aggregate values of categorical constraint variables are already
known from the Census, so are bound to be accurate. Checking the distribution
of continuous variables such as age and distance travelled to work against
these crude categories is problematic.\footnote{For example, if 50\% of
...
national distance trends within each distance bin.
  ...
  \item Target variables are, almost by definition, not known as geographic
aggregates (otherwise they would be constraints). Therefore checking their
validity for small areas is difficult due to data limitations.
...
determined entirely by their relationships with constraint variables. However,
...
distance travelled to work, yet negatively correlated to this variable in
another.
...
correlation between unconstrained `interaction' variables --- i.e. variables
composed of two or more constraint variables (e.g. female driver).

The first approach tested the model's ability to simulate income. Accurate
income data are lacking for small areas. However, Neighbourhood Statistics
models net and gross household incomes and the MSOA level. For the purposes of
...
The purpose of this fitting exercise is not to provide accurate income
estimates at the local level, based on census statistics, however.
It is to evaluate the
ability of the spatial microsimulation model. In terms of income, a variable
...
questions that merit further attention based on available (and in the first
case, very limited) data.
...
data is. How much have Nomis data been randomised, and in what way? This
question is relatively easy to answer because of the shared variables between
census variables published by Nomis and Casweb at the MSOA level. Scatter plots
suggest Nomis data is faithful to the original census results:
...
to to a fixed randomising factor.  To test this, the errors were plotted for
...
by randomisation is constant for each variable (this was confirmed by plotting
the errors for additional census variables). QQ plots suggest that the
...
\begin{itemize}
 ...
 validate the spatial microdata locally.
 \item Comparing of the spatial microdata over entire region with a
 survey data that specifies home region of resident.
 \item Aggregating local model outputs to a coarser geographical levels at which
 cross-tabulated data is available.
...
otherwise know, it is hard to find reliable individual-level
...
other socio-economic details. This would allow the model to be evaluated
...
rather than just counts of (often rather coarse) categories. T-tests and
...
to model individuals. Yet, as we have seen in the previous section, the IPF
...
integer results. \citet{Lovelace2013-arxiv} tackled this issue in detail and
developed a new method of integerisation. The following section is therefore
based on \citet{Lovelace2013-arxiv} and repeats much of the content.
...
reweighting and the benefits of using whole cases.
%allows for the
...
characteristics are not recorded at the local level. Intergerisation translates
...
`proportional probabilities' and finally `truncate, replicate, sample'. TRS
builds on the strengths of the other methods, hence the order in which they are
presented.

The application of these methods
to the same dataset (and their implementation in the same language, R) allows
...
It is important to clarify what we mean by `weights' before proceeding to
...
replicated (Ballas et al., 2005a): to ensure replication weights do not differ
...
the decimal is 0.5 or above, the integer is rounded up; if not, the
integer is rounded down.
...
populations such that $Pop_{sim} \geq Pop_{cens}$. To do this, an inclusion
threshold ($IT$) is created, set to 1 and then iteratively
reduced (by 0.001 each time), adding extra individuals with
incrementally lower weights.\footnote{A more detailed description of the steps
...
To overcome this problem, we took the truncated weights as the starting
population, rather than the rounded weights. This modified approach improved the
accuracy of the integer results and is therefore what we refer to when
the `threshold approach' is mentioned
henceforth.\footnote{An
...
methods are equally easy to implement, we henceforth refer only to the superior
...
\begin{figure}[h]
 \centerline{
 \includegraphics[width=14 cm]{IPF-4c2.pdf}}
 % hist-weights.pdf: 792x612 pixel, 72dpi, 27.94x21.59 cm, bb=0 0 792 612
...
provided in the Supplementary Information, we encourage others to use, build-on
